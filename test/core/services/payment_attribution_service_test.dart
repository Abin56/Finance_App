...
void main() {
  ...
  setUp(() {
    ...
    service = PaymentAttributionService(
  ...
  group(
    'PaymentAttributionService.apply — EMI installments (full/partial/advance/overdue)',
    () {
      ...
      test('full payment: a friend pays one installment in full', () async {
        ...
        expect(
          refreshedAlice!.currentBalance,
          -installment.amountDue,
          reason: 'you now owe Alice',
        );
      ...
      test(
        'partial payment: a friend pays less than the full installment',
        () async {
          final alice = await personRepository.createPerson(
            name: 'Alice',
            avatarColorValue: 0xFF5B5FEF,
      ...
      test(
        'advance payment: a friend pays an installment ahead of its scheduled turn',
        () async {
          final alice = await personRepository.createPerson(
            name: 'Alice',
            avatarColorValue: 0xFF5B5FEF,
          ...
          expect(
            ...
            0,
            reason:
                'installment 1 has not been paid — this payment jumps ahead of it',
          );
          ...
          final refreshedEarlier = await installmentRepositoryFor(
            emi.scheduleId,
          ).getByKey(earlier.id);
          expect(
            refreshedEarlier!.amountPaid,
            0,
            reason: 'paying ahead does not touch earlier installments',
          );
        },
      );

      test(
        'overdue payment: a friend pays an installment already past its due date',
        () async {
          final alice = await personRepository.createPerson(
            name: 'Alice',
            avatarColorValue: 0xFF5B5FEF,
          ...
          expect(
            overdueInstallment.status,
            InstallmentStatus.overdue,
            reason: 'startDate is in 2020, so installment 1 is overdue today',
          );
          ...
          final refreshed = await installmentRepositoryFor(
            emi.scheduleId,
          ).getByKey(overdueInstallment.id);
          expect(refreshed!.status, InstallmentStatus.paid);
        },
      );

      test(
        'multiple installments paid together post ONE combined LedgerEntry',
        () async {
          final alice = await personRepository.createPerson(
            name: 'Alice',
            avatarColorValue: 0xFF5B5FEF,
          ...
          final refreshedFirst = await installmentRepositoryFor(
            emi.scheduleId,
          ).getByKey(first.id);
          final refreshedSecond = await installmentRepositoryFor(
            emi.scheduleId,
          ).getByKey(second.id);
          ...
          expect(
            refreshedAlice!.currentBalance,
            -(first.amountDue + second.amountDue),
          );
          ...
          expect(
            ledgerSnapshot.docs,
            hasLength(1),
            reason: 'one combined entry, not one per installment',
          );
          expect(
            (ledgerSnapshot.docs.single.data()['amount'] as num).toDouble(),
            first.amountDue + second.amountDue,
          );
        },
      );
      ...
      test(
        'you paying yourself (PayerSource.self) posts no LedgerEntry',
        () async {
          final installment = await installmentAt(1);
  ...
  group('PaymentAttributionService.apply — Bill payments', () {
    ...
    setUp(() async {
      ...
      occurrence = await occurrenceRepository.ensureCurrentOccurrence(
        bill,
        const [],
      );
    });
    ...
    test(
      'a friend pays part of a bill (partial payment) and the person ledger updates',
      () async {
    ...
    test('a friend pays a one-time bill in full', () async {
      ...
      final oneTimeOccurrenceRepository = occurrenceRepositoryFor(
        oneTimeBill.id,
      );
      final oneTimeOccurrence = await oneTimeOccurrenceRepository
          .ensureCurrentOccurrence(oneTimeBill, const []);
      ...
      expect(
        descriptions.single,
        'Bob paid ₹2,000.00 towards your Laptop repair bill',
      );
    });
  });
}
