...
void main() {
  ...
  for (final scale in _scales) {
    testWidgets(
      'UpcomingDueRow renders carried-over and normal items without overflow @${scale}x',
      (tester) async {
        await pumpAt(
          tester,
          scale,
          Column(
            children: [
              UpcomingDueRow(item: creditCardItem),
              UpcomingDueRow(item: billItem),
            ],
          ),
        );
  ...
  for (final scale in _scales) {
    testWidgets(
      'UpcomingDueBreakdownSheet renders a Credit Card breakdown without overflow @${scale}x',
      (tester) async {
        await pumpAt(
          tester,
          scale,
          ...
          overrides: [
            upcomingDueBreakdownProvider.overrideWith(
              (ref, item) => const CreditCardBreakdown(
                totalAmount: 1234567.89,
                othersShare: 456789.12,
                transactionCount: 47,
              ),
            ),
          ],
  ...
  for (final scale in _scales) {
    testWidgets(
      'UpcomingDueBreakdownSheet renders a no-interest EMI breakdown without overflow @${scale}x',
      (tester) async {
        final emiItem = (
