...
void main() {
  ...
  group('InstallmentCycleItem field mapping', () {
    test('sourceType maps every OwnerType to its matching CycleSourceType', () {
      expect(
        InstallmentCycleItem(
          installment(ownerType: OwnerType.loan, dueDate: DateTime(2026, 7, 1)),
        ).sourceType,
        CycleSourceType.loan,
      );
      expect(
        InstallmentCycleItem(
          installment(ownerType: OwnerType.emi, dueDate: DateTime(2026, 7, 1)),
        ).sourceType,
        CycleSourceType.emi,
      );
      expect(
      ...
      expect(
        InstallmentCycleItem(
          installment(ownerType: OwnerType.bill, dueDate: DateTime(2026, 7, 1)),
        ).sourceType,
    ...
    test(
      'cycleDate/totalAmount/paidAmount pass through from the wrapped Installment',
      () {
    ...
    test('isSettled is true when skipped, even if unpaid', () {
      final item = InstallmentCycleItem(
        installment(dueDate: DateTime(2026, 7, 1), isSkipped: true),
      );
      expect(item.isSettled, isTrue);
    });
    ...
    test('isOverdue reflects Installment.status == overdue', () {
      final overdue = InstallmentCycleItem(
        installment(dueDate: DateTime(2020, 1, 1)),
      );
      final upcoming = InstallmentCycleItem(
        installment(dueDate: DateTime(2999, 1, 1)),
      );
    ...
    test('carryForwardEligible defaults to true (not overridden)', () {
      final item = InstallmentCycleItem(
        installment(dueDate: DateTime(2026, 7, 1)),
      );
  ...
  group('InstallmentCycleItem + CycleEngine.classifyForCarryForward', () {
    ...
    test('an unpaid previous-cycle installment carries forward', () {
      final item = InstallmentCycleItem(
        installment(dueDate: DateTime(2026, 7, 5)),
      );
    ...
    test(
      'a current-cycle installment always shows regardless of paid state',
      () {
        final item = InstallmentCycleItem(
          installment(
