...
void main() {
  group('CycleAggregate.from', () {
    test(
      'sums remainingAmount across previousCyclePending and current separately',
      () {
        final result = CycleEngineResult<_FakeCycleItem>(
          previousCyclePending: [
            _FakeCycleItem(
              id: 'a',
              sourceType: CycleSourceType.bill,
              totalAmount: 100,
              paidAmount: 40,
            ),
            _FakeCycleItem(
              id: 'b',
              sourceType: CycleSourceType.emi,
              totalAmount: 200,
              paidAmount: 0,
            ),
          ],
          current: [
            _FakeCycleItem(
              id: 'c',
              sourceType: CycleSourceType.loan,
              totalAmount: 50,
              paidAmount: 50,
            ),
          ],
    ...
    test(
      'items with null totalAmount/paidAmount contribute zero, not an error',
      () {
        final result = CycleEngineResult<_FakeCycleItem>(
          previousCyclePending: [
            _FakeCycleItem(
              id: 'a',
              sourceType: CycleSourceType.manualAdjustment,
            ),
          ],
          current: [],
          future: [],
    ...
    test(
      'countBySourceType tallies previousCyclePending + current combined, per source type',
      () {
        final result = CycleEngineResult<_FakeCycleItem>(
          previousCyclePending: [
          ...
          future: [_FakeCycleItem(id: 'e', sourceType: CycleSourceType.bill)],
        );
