...
void main() {
  ...
  setUp(() {
    ...
    final installmentCollection = firestore
  ...
  group('InstallmentRepository.generateInstallments — even split', () {
    test(
      'oneTime creates exactly 1 installment with amountDue == totalAmount',
      () async {
    ...
    test(
      'monthly + fixed installmentCount splits evenly with correct due-date spacing',
      () async {
        final schedule = await createSchedule(
          totalAmount: 300,
    ...
    test(
      'odd amount: last installment absorbs the rounding remainder, total matches exactly',
      () async {
        final schedule = await createSchedule(
          totalAmount: 100,
          scheduleType: ScheduleType.monthly,
    ...
    test('throws AppException when installmentCount is null', () async {
      ...
      await expectLater(
        installmentRepository.generateInstallments(schedule),
  ...
  group('InstallmentRepository.generateInstallments — precomputed amounts', () {
    test(
      'stamps exact amounts and principal/interest portions when provided',
      () async {
        final schedule = await createSchedule(
          totalAmount: 210,
          scheduleType: ScheduleType.monthly,
    ...
    test(
      'throws AppException when precomputedAmounts length mismatches installmentCount',
      () async {
        final schedule = await createSchedule(
          totalAmount: 210,
          scheduleType: ScheduleType.monthly,
        ...
        await expectLater(
          installmentRepository.generateInstallments(
            schedule,
            precomputedAmounts: const [
              PrecomputedInstallmentAmount(amountDue: 210),
            ],
          ),
          throwsA(isA<AppException>()),
        );
  ...
  group('InstallmentRepository.generateInstallments — dueDayOfMonth', () {
    test(
      'installment #1 stays exactly on firstDueDate even when its day differs from dueDayOfMonth',
      () async {
        final schedule = await createSchedule(
          totalAmount: 400,
          scheduleType: ScheduleType.monthly,
    ...
    test(
      'installments #2+ snap to the fixed dueDayOfMonth every month — matches the worked example',
      () async {
    ...
    test(
      'clamps to the last valid day in shorter months (e.g. due day 31 in February/30-day months)',
      () async {
        final schedule = await createSchedule(
          totalAmount: 400,
          scheduleType: ScheduleType.monthly,
        ...
        expect(
          sorted[2].dueDate,
          DateTime(2026, 3, 31),
        ); // reverts to 31 once the month allows it
        expect(
          sorted[3].dueDate,
          DateTime(2026, 4, 30),
    ...
    test(
      'when firstDueDate.day already equals dueDayOfMonth, schedule is identical to the no-argument case',
      () async {
        final schedule = await createSchedule(
          totalAmount: 300,
          scheduleType: ScheduleType.monthly,
        ...
        final withDueDay = await installmentRepository.generateInstallments(
          schedule,
          dueDayOfMonth: 5,
        );
        final sorted = [...withDueDay]
          ..sort((a, b) => a.sequenceNumber.compareTo(b.sequenceNumber));
    ...
    test(
      'null dueDayOfMonth behaves identically to before this parameter existed (regression guard)',
      () async {
        final schedule = await createSchedule(
          totalAmount: 300,
          scheduleType: ScheduleType.monthly,
        ...
        expect(
          sorted[2].dueDate,
          DateTime(2026, 3, 28),
        ); // chained from Feb 28, not reset to 31
      },
    );
  ...
  group('InstallmentRepository.applyPayment', () {
    test('clamps amountPaid to amountDue and records an audit entry', () async {
      ...
      final installment = (await installmentRepository.generateInstallments(
        schedule,
      )).single;
  ...
  group('Installment.status', () {
    ...
    test(
      'partiallyPaid when 0 < amountPaid < amountDue regardless of due date',
      () {
        final installment = _installment(
          dueDate: DateTime.now().subtract(const Duration(days: 5)),
          amountPaid: 50,
  ...
  group('InstallmentRepository.closeOutRemaining', () {
    test(
      'soft-deletes not-fully-paid installments, leaves fully paid ones untouched',
      () async {
        final schedule = await createSchedule(
          totalAmount: 300,
          scheduleType: ScheduleType.monthly,
        ...
        final closed = await installmentRepository.closeOutRemaining(
          installments,
        );

        expect(
          closed.map((i) => i.id),
          containsAll([installments[1].id, installments[2].id]),
        );
  ...
  group('InstallmentRepository.replaceUnpaid', () {
    test(
      'soft-deletes only installments with zero payment, leaves paid and partially-paid alone',
      () async {
        final schedule = await createSchedule(
          totalAmount: 300,
          scheduleType: ScheduleType.monthly,
        ...
        final replaced = await installmentRepository.replaceUnpaid(
          installments,
        );
        ...
        final remainingAfter = await installmentRepository.getAll();
        expect(
          remainingAfter.map((i) => i.id),
          containsAll([installments[0].id, installments[1].id]),
        );
    ...
    test(
      'is a no-op when every installment already carries a payment',
      () async {
        final schedule = await createSchedule(
          totalAmount: 100,
          scheduleType: ScheduleType.oneTime,
  ...
  group('InstallmentRepository query helpers', () {
    test(
      'thisMonth/nextMonth/future/overdue correctly partition installments',
      () {
        final now = DateTime(2026, 3, 15);
        final all = [
          _installment(
            dueDate: DateTime(2026, 3, 20),
          ), // this month, not yet due
    ...
    test(
      'thisWeek returns installments due within the current Monday-Sunday week',
      () {
    ...
    test(
      'remainingAmount sums correctly and excludes skipped installments',
      () {
        final all = [
