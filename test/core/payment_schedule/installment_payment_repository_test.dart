...
void main() {
  ...
  setUp(() async {
    ...
    installment = (await installmentRepository.generateInstallments(
      schedule,
    )).single;
  });
  ...
  group('InstallmentPaymentRepository.recordPayment', () {
    ...
    test(
      'advance/early payment (before due date) is accepted with no special validation',
      () async {
        final earlyDate = installment.dueDate.subtract(
    ...
    test(
      'records settlementMethod/billingCycleLabel and computes remainingBalanceAfterPayment',
      () async {
    ...
    test(
      'settlementMethod/billingCycleLabel default to null when not supplied',
      () async {
        final payment = await paymentRepository.recordPayment(
          installment,
          amount: 400,
    ...
    test(
      'multiple partial payments each record their own remainingBalanceAfterPayment',
      () async {
  ...
  group('InstallmentPaymentRepository.softDeletePayment / restorePayment', () {
    test(
      'softDeletePayment reverses amountPaid and moves the payment to trash',
      () async {
        final payment = await paymentRepository.recordPayment(
          installment,
          amount: 400,
    ...
    test(
      'restorePayment re-applies the effect and restores from trash',
      () async {
        final payment = await paymentRepository.recordPayment(
          installment,
          amount: 400,
