...
void main() {
  ...
  group('Feature 1 — cashFlowDateRangeProvider / cashFlowForRangeProvider', () {
    test('a custom range includes transactions inside it', () async {
      ...
      container
          .read(cashFlowDateRangeProvider.notifier)
          .state = CashFlowPeriod.custom(
    ...
    test('excludes a transaction dated before the selected range', () async {
      ...
      expect(
        cashFlow.moneyOut,
        0,
        reason: 'dated before the range — must be excluded',
      );
    });
    ...
    test('excludes a transaction dated after the selected range', () async {
      ...
      expect(
        cashFlow.moneyOut,
        0,
        reason: 'dated after the range — must be excluded',
      );
    });
    ...
    test('changing the selected range changes the provider output', () async {
      ...
      expect(
        ...
        250,
        reason: 'switching range must recompute, not cache the old one',
      );
    });
  ...
  group('Feature 2 — myExpensesForRangeProvider', () {
    ...
    test(
      '₹300 split equally among 3 (including me) counts only my ₹100 share',
      () async {
        final accountId = await createAccount(container);
        ...
        expect(
          breakdown.total,
          100,
          reason: 'only my ₹100 share of the ₹300 dinner, not the full ₹300',
        );
    ...
    test(
      'a custom split uses my explicit assigned amount, not an equal share',
      () async {
        final accountId = await createAccount(container);
        final expenses = container.read(expenseRepositoryProvider);
        await expenses.createExpense(
        ...
        expect(
          ...
          400,
          reason: 'my explicit ₹400, not an equal ₹333 share',
        );
      },
    );
    ...
    test('other participants\' shares are excluded from My Expenses', () async {
      ...
      expect(
        container.read(myExpensesForRangeProvider).total,
        300,
        reason: 'the other ₹600 must never appear as my expense',
      );
    });

    test(
      'a shared expense is not double-counted across personal + split',
      () async {
        final accountId = await createAccount(container);
        final expenses = container.read(expenseRepositoryProvider);
        await expenses.createExpense(
    ...
    test(
      'full scenario: personal + split share included, EMI/bills excluded',
      () async {
        final accountId = await createAccount(container);
        final expenses = container.read(expenseRepositoryProvider);
        ...
        expect(
          breakdown.total,
          600,
          reason: '₹500 personal + ₹100 my share of dinner = ₹600',
        );
      },
    );

    test(
      'a transaction marked excludeFromCalculations does not count toward My Expenses',
      () async {
        final accountId = await createAccount(container);
        final transactions = container.read(transactionRepositoryProvider);
        await transactions.createTransaction(
    ...
    test(
      'a transaction reassigned to a different accountingMonth is bucketed by that month for a whole-month preset',
      () async {
        ...
        expect(
          ...
          700,
          reason:
              'a custom range is day-precision — it buckets by the real transaction date, not accountingMonth',
        );
        ...
        expect(
          container.read(myExpensesForRangeProvider).total,
          0,
          reason: 'dateTime is in September, outside this October custom range',
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        container.read(cashFlowDateRangeProvider.notifier).state =
            const CashFlowPeriod.preset(CashFlowPreset.thisMonth);
        expect(
          container.read(myExpensesForRangeProvider).total,
          0,
          reason: 'reassigned to next month — must not count in this month',
        );
      },
    );
  });
