...
void main() {
  ...
  Future<void> materializeBill(
    ProviderContainer container,
    String billId,
  ) async {
    final sub = container.listen(
      materializeBillOccurrenceProvider(billId),
      (_, _) {},
    );
  ...
  Future<BillOccurrence> recordBillPayment(
    ProviderContainer container,
    Bill bill, {
  ...
  group('totalDueThisMonthProvider', () {
    test(
      'sums due/paid/remaining across this-month bills only (current month only)',
      () async {
    ...
    test(
      'still counts an unpaid bill carried over from a prior month (overdue only)',
      () async {
        final now = DateTime.now();
        final bills = container.read(billRepositoryProvider);
        ...
        expect(
          ...
          500,
          reason:
              'an overdue carry-over must still show up as due, not be dropped',
        );
      },
    );

    test(
      'sums current-month due plus prior-month overdue (current month + overdue)',
      () async {
        final now = DateTime.now();
        final bills = container.read(billRepositoryProvider);
    ...
    test(
      'excludes a prior-month bill that has since been fully paid off (paid overdue)',
      () async {
        final now = DateTime.now();
        final bills = container.read(billRepositoryProvider);
        ...
        expect(
          breakdown.due,
          0,
          reason: 'a fully-paid overdue bill must not be counted as still due',
        );
        expect(breakdown.paid, 0);
      },
    );
    ...
    test(
      'never double-counts a bill that already belongs to this month (duplicate prevention)',
      () async {
        final now = DateTime.now();
        final bills = container.read(billRepositoryProvider);
        ...
        expect(
          breakdown.due,
          1000,
          reason: 'billA must be counted exactly once, not once per branch',
        );
      },
    );

    test(
      'EMI installment breakdown also merges an unpaid prior-month installment',
      () async {
        final now = DateTime.now();
        ...
        await container.read(
          installmentsStreamProvider(createdEmi.scheduleId).future,
        );
        ...
        expect(
          ...
          1200,
          reason:
              'an overdue EMI installment from a prior month must still be counted',
        );
  ...
  group('totalMoneyToReceiveProvider', () {
    test(
      'sums People-ledger and Lending receivables independently, no double-count',
      () async {
  ...
  group('upcomingPaymentsTimelineProvider', () {
    test(
      'sorts overdue items before upcoming items regardless of relative dates',
      () async {
        final now = DateTime.now();
        final bills = container.read(billRepositoryProvider);
  ...
  group('cashFlowThisMonthProvider', () {
    test(
      'moneyOut adds bill payments on top of expense transactions, not double-counted',
      () async {
        final now = DateTime.now();
    ...
    test(
      'excludes a transaction marked excludeFromCalculations from both moneyIn and moneyOut',
      () async {
        final now = DateTime.now();
        final accounts = container.read(accountRepositoryProvider);
        final account = await accounts.createAccount(
        ...
        expect(
          cashFlow.moneyOut,
          0,
          reason: 'excluded expense must not count toward Money Out',
        );
      },
    );

    test(
      'a transaction assigned to next month via accountingMonth is excluded from this month\'s cash flow',
      () async {
        final now = DateTime.now();
        ...
        expect(
          cashFlow.moneyOut,
          0,
          reason: 'reassigned to next month — must not count toward this month',
        );
      },
    );
  });
