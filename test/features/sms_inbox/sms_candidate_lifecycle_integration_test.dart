...
void main() {
  ...
  setUp(() async {
    ...
    cloudCandidateRepository = SmsTransactionCandidateRepository(candidateCollection);
    cloudSync = SmsCandidateCloudSync(cloudCandidateRepository, candidateDao, inboxRepository);
  });
  ...
  Future<void> scanAndBuildCandidates(List<RawSmsMessage> messages, AccountCardMatcher matcher) async {
  ...
  test(
    'full pipeline: real SMS -> parsed -> matched candidate -> cloud doc with the exact '
    'web-readable shape -> conversion -> immediate cloud cleanup',
    () async {
      ...
      final matcher = AccountCardMatcher(accounts: [account], cards: const <CreditCardProfile>[]);

      final message = RawSmsMessage(
        address: 'VM-HDFCBK',
        body: 'Rs.1,250.50 debited from a/c XX5623 on 15-07-26 to VPA swiggy@icici. Ref No 123456789012.',
      ...
      expect(smsItem.parsed, isNotNull, reason: 'the bank-specific HDFC parser must recognize this message');

      final localCandidate = await candidateDao.getBySmsItemId(smsItem.id);
      expect(localCandidate, isNotNull);
      expect(localCandidate!.matchedAccountId, accountId, reason: 'last-4 5623 must resolve to the seeded account');
      ...
      final snapshot = await firestore.collection('smsTransactionCandidates').doc(smsItem.id).get();
      final raw = snapshot.data()!;

      expect(snapshot.id, smsItem.id, reason: 'the document id must be the local SmsInboxItem id, not a random id');
      ...
      for (final forbiddenKey in ['body', 'rawBody', 'sender', 'address', 'userId', 'candidateStatus']) {
      ...
      expect(refreshedAccount!.currentBalance, 5000 - 1250.5, reason: 'the real balance-adjustment path must still run');
      ...
      expect(cloudAfterConvert, isEmpty, reason: 'the cloud candidate must be gone immediately, not just after the next scan()');
  ...
  test(
    ...
    () async {
      ...
      final throwingCloudRepository = _ThrowingDeleteRepository(cloudCandidateRepository.collection);
      ...
      expect(smsAfterFailedCleanup.status, SmsImportStatus.imported, reason: 'local marking must not roll back on cloud failure');
