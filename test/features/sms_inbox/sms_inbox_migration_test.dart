...
void main() {
  ...
  test(
    'preserves existing rows, their status and their linked financial record',
    () async {
  ...
  test(
    'backfills a real message key, so a re-scan recognizes the migrated row',
    () async {
      ...
      expect(
        migrated.messageKey,
        SmsMessageKey.compute(
          sender: existing.address,
          dateTime: existing.date,
          body: existing.body,
        ),
        reason:
            'a placeholder key would not match the next scan of the same physical message',
      );
      ...
      final repository = SmsInboxRepository(
        dao,
        _FakeSmsReaderAdapter([existing]),
      );
