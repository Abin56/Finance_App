...
void main() {
  ...
  RawSmsMessage msg(String address, String body, {DateTime? date}) =>
      RawSmsMessage(
        address: address,
        body: body,
        date: date ?? DateTime(2026, 7, 15, 14, 45),
      );
  ...
  group('SmsFinancialFilter.isFinancial', () {
    test('rejects OTP messages', () {
      final m = msg(
        'VM-HDFCBK',
        '123456 is your OTP for txn of Rs.500. Do not share OTP with anyone.',
      );
    ...
    test('rejects promotional messages', () {
      final m = msg(
        'AD-OFFERS',
        'Flat 50% off on electronics! Shop now, sale is now live.',
      );
      expect(SmsFinancialFilter.isFinancial(m), isFalse);
    });
    ...
    test('rejects delivery updates', () {
      final m = msg(
        'VM-DELIVR',
        'Your order #12345 has been dispatched and will be delivered tomorrow.',
      );
      expect(SmsFinancialFilter.isFinancial(m), isFalse);
    });
    ...
    test('rejects recharge reminders', () {
      final m = msg(
        'VM-AIRTEL',
        'Your plan expires soon. Recharge now to continue enjoying uninterrupted service.',
      );
      expect(SmsFinancialFilter.isFinancial(m), isFalse);
    });
    ...
    test('accepts a genuine credit alert', () {
      final m = msg(
        'VM-ICICIB',
        'Rs.50,000.00 credited to a/c XX1234 on 15-07-26. Info: NEFT salary. Avl Bal Rs.75,000.00',
      );
      expect(SmsFinancialFilter.isFinancial(m), isTrue);
    });

    test(
      'accepts a genuine debit whose merchant name happens to contain "Sale Store"',
      () {
    ...
    test(
      'still rejects a promo that merely quotes an amount with no transaction verb',
      () {
        final m = msg(
          'AD-OFFERS',
          'Get a cashback offer of Rs.500 off your next purchase! Sale is now live.',
        );
        expect(SmsFinancialFilter.isFinancial(m), isFalse);
      },
    );
    ...
    test(
      'OTP is rejected even if the message also contains a transaction verb',
      () {
        final m = msg(
          'VM-HDFCBK',
          '123456 is your OTP to authorize a payment of Rs.500 debited from a/c XX1234. Do not share OTP.',
        );
        expect(SmsFinancialFilter.isFinancial(m), isFalse);
      },
    );
  ...
  group('bank-specific parsing', () {
    ...
    test('parses an SBI ATM withdrawal', () {
      final m = msg(
        'AX-SBIBNK',
        'Rs.5,000.00 withdrawn at ATM from a/c XX9876 on 15-07-26. -SBI',
      );
      final result = registry.tryParse(m);
    ...
    test('parses an Axis Bank credit card purchase', () {
      final m = msg(
        'JM-AXISBK',
        'Rs.2,499.00 spent on your Axis Bank Credit Card ending 4321 at AMAZON on 15-07-26.',
      );
      final result = registry.tryParse(m);
  ...
  group('generic fallback parsing (unrecognized bank+no UPI keyword)', () {
    test('still extracts amount/direction from an unrecognized-format alert', () {
      final m = msg(
        'BK-UNKNWN',
        'INR 750.50 debited from your account for bill payment. Avl bal INR 4,250.00.',
      );
      final result = registry.tryParse(m);
  ...
  group('amount formatting edge cases', () {
    test('handles amount without decimal places', () {
      final m = msg(
        'VM-HDFCBK',
        'Rs.500 debited from a/c XX1111 towards UPI payment.',
      );
      final result = registry.tryParse(m);
