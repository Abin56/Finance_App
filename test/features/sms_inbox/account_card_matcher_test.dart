...
void main() {
  ...
  test('resolves a credit card by last-4 and confirms the bank', () {
    ...
    final result = matcher.match(parsed(bankName: 'HDFC Bank', maskedAccountOrCard: '1234'));
  ...
  test('resolves a plain bank account by last-4 (SBI)', () {
    final sbiAccount = account(id: 'acc-2', name: 'SBI Savings', bankId: 'sbi', last4: '5678');
    final matcher = AccountCardMatcher(accounts: [sbiAccount], cards: const []);

    final result = matcher.match(parsed(bankName: 'State Bank of India', maskedAccountOrCard: '5678'));
  ...
  test('resolves by last-4 alone when the bank cannot be confirmed', () {
    ...
    final result = matcher.match(parsed(bankName: null, maskedAccountOrCard: '1234'));
  ...
  test('flags a bank mismatch as still resolved but not bank-confirmed', () {
    ...
    final result = matcher.match(parsed(bankName: 'ICICI Bank', maskedAccountOrCard: '1234'));
  ...
  test('never guesses when two cards share the same last-4', () {
    ...
    final matcher = AccountCardMatcher(accounts: [acc1, acc2], cards: [card1, card2]);
  ...
  test('a bank match with no last-4 is only ever a suggestion, never resolved', () {
    final hdfcAccount = account(id: 'acc-1', name: 'HDFC Savings', bankId: 'hdfc', last4: '9999');
    final matcher = AccountCardMatcher(accounts: [hdfcAccount], cards: const []);

    final result = matcher.match(parsed(bankName: 'HDFC Bank', maskedAccountOrCard: null));
  ...
  test('no signal at all matches nothing', () {
    ...
    final result = matcher.match(parsed(bankName: 'Axis Bank', maskedAccountOrCard: '4321'));

    expect(result.isResolved, isFalse);
    expect(result.alternatives, isEmpty);
  });

  test('an account already represented by its card is not also offered as a plain account', () {
    ...
    final hdfcAccount = account(id: 'acc-1', name: 'HDFC Card', bankId: 'hdfc', last4: '1234');
    final hdfcCard = card(id: 'card-1', accountId: 'acc-1', last4: '1234');
    final matcher = AccountCardMatcher(accounts: [hdfcAccount], cards: [hdfcCard]);
