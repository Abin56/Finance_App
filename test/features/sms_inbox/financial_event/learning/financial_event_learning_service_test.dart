...
void main() {
  ...
  group('confirmation', () {
    ...
    test(
      'first user confirmation of a suggested value increments confirmations',
      () async {
        ...
        expect(
          profile.category.corrections,
          1,
          reason: 'the AI-sourced write still counts as the sole correction',
        );
      },
    );

    test(
      'repeated confirmation increments confirmations without any correction event',
      () async {
        await repo.applyCorrection<String>(
          userId: 'u1',
          merchantKey: 'swiggy',
        ...
        final action = const UserLearningAction(
          userId: 'u1',
          rawMerchantName: 'Swiggy',
          category: 'food_delivery',
        );
    ...
    test(
      'confirmation of a user-set value creates no correction history',
      () async {
  ...
  group('correction', () {
    test(
      'first correction on an empty profile records exactly one correction event',
      () async {
    ...
    test(
      'correction chain Shopping -> Food -> Grocery preserves full history',
      () async {
        await service.correctFinancialEventClassification(
          const UserLearningAction(
            userId: 'u1',
    ...
    test(
      'individual field corrections: category, merchantType, paymentProvider, paymentMethod, subcategory',
      () async {
    ...
    test(
      'same-value correction is treated as a confirmation, not a meaningless correction',
      () async {
        await service.correctFinancialEventClassification(
          const UserLearningAction(
            userId: 'u1',
        ...
        expect(
          history,
          hasLength(1),
          reason: 'no second correction event for an unchanged value',
        );
    ...
    test(
      'correction-after-confirmation then confirmation-after-correction sequences',
      () async {
        const action1 = UserLearningAction(
          userId: 'u1',
          rawMerchantName: 'Amazon',
          category: 'shopping',
        );
        ...
        const action2 = UserLearningAction(
          userId: 'u1',
          rawMerchantName: 'Amazon',
          category: 'electronics',
        );
        final correctResult = await service.correctFinancialEventClassification(
          action2,
          at: DateTime(2026, 1, 2),
        );
        expect(correctResult.correctedFields, [LearnedFieldType.category]);

        final confirmResult = await service.confirmFinancialEventClassification(
          action2,
          at: DateTime(2026, 1, 3),
        );
  ...
  group('provider guard', () {
    test(
      'a payment provider name is rejected as a merchant identity for confirmation',
      () async {
        expect(
          () => service.confirmFinancialEventClassification(
            const UserLearningAction(
              userId: 'u1',
              rawMerchantName: 'PhonePe',
              category: 'transfers',
            ),
          ),
          throwsArgumentError,
    ...
    test(
      'a payment provider name is rejected as a merchant identity for correction',
      () async {
        expect(
          () => service.correctFinancialEventClassification(
            const UserLearningAction(
              userId: 'u1',
              rawMerchantName: 'Google Pay',
              category: 'transfers',
            ),
          ),
          throwsArgumentError,
  ...
  group('merchant key normalization', () {
    test(
      'Swiggy, SWIGGY, and "Swiggy Pvt Ltd" normalize to the same profile',
      () async {
        await service.correctFinancialEventClassification(
          const UserLearningAction(
            userId: 'u1',
        ...
        await service.confirmFinancialEventClassification(
          const UserLearningAction(
            userId: 'u1',
            rawMerchantName: 'Swiggy Pvt Ltd',
            category: 'food',
          ),
          at: DateTime(2026, 1, 3),
        );
    ...
    test('Swiggy and Swiggy Instamart stay distinct profiles', () async {
      ...
      await service.correctFinancialEventClassification(
        const UserLearningAction(
          userId: 'u1',
          rawMerchantName: 'Swiggy Instamart',
          category: 'groceries',
        ),
        at: DateTime(2026, 1, 1),
      );
      ...
      expect(
        (await repo.getProfile('u1', 'swiggy'))!.category.value,
        'food_delivery',
      );
      expect(
        (await repo.getProfile('u1', 'swiggy instamart'))!.category.value,
        'groceries',
      );
    });
  });
  ...
  group('isolation and independence', () {
    test(
      'two users acting on the same merchant name never see each other\'s data',
      () async {
        await service.correctFinancialEventClassification(
          const UserLearningAction(
            userId: 'u1',
        ...
        expect(
          (await repo.getProfile('u1', 'amazon'))!.category.value,
          'shopping',
        );
        expect(
          (await repo.getProfile('u2', 'amazon'))!.category.value,
          'electronics',
        );
        expect(await repo.getCorrectionHistory('u1', 'amazon'), hasLength(1));
    ...
    test('correcting one field never mutates unrelated fields', () async {
      ...
      expect(
        after.paymentProvider.corrections,
        before.paymentProvider.corrections,
      );
    });
  });
  ...
  group('empty / null actions', () {
    test(
      'an action naming no fields is a safe no-op for confirmation',
      () async {
        final result = await service.confirmFinancialEventClassification(
  ...
  group('atomic-failure leaves no partial state', () {
    test(
      'a failure mid-transaction leaves the service\'s view of the profile unchanged',
      () async {
        ...
        try {
          await dao.transaction((txnDao) async {
            ...
            throw StateError(
              'simulated failure before correction history is appended',
            );
          });
        } catch (e) {
          caught = e;
        ...
        expect(
          ...
          'shopping',
          reason: 'partial transaction must have rolled back',
        );
        expect(await repo.getCorrectionHistory('u1', 'amazon'), hasLength(1));
      },
    );
  ...
  group('structural exclusion of hard SMS evidence', () {
    test(
      'UserLearningAction has no way to carry amount/direction/account/status/reference/raw text',
      () {
        ...
        const action = UserLearningAction(
          userId: 'u1',
          rawMerchantName: 'Swiggy',
          category: 'food',
        );
