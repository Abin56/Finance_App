...
void main() {
  ...
  group('LearningConfidence.compute', () {
    ...
    test(
      'a field with many corrections relative to confirmations is less confident',
      () {
    ...
    test(
      'AI/inference sourced values are trusted less than user-sourced ones',
      () {
        ...
        final userConfidence = LearningConfidence.compute(
          field: userField,
          now: now,
        );
        final aiConfidence = LearningConfidence.compute(
          field: aiField,
          now: now,
        );
        final inferredConfidence = LearningConfidence.compute(
          field: inferredField,
          now: now,
        );
    ...
    test(
      'a stale value (untouched past staleAfter) has discounted confidence',
      () {
