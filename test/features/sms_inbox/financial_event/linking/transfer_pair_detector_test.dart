...
void main() {
  ...
  test(
    'returns null for an event not flagged as an own-account transfer',
    () async {
  ...
  test(
    'Safety rules 6-7: pairs the debit and credit legs, neither becomes income or expense',
    () async {
      ...
      final creditLeg = buildEvent(
        id: 'credit',
        eventDate: base.add(const Duration(minutes: 2)),
        amount: 20000,
  ...
  test(
    'a single unpaired leg stays unresolved, never income or expense',
    () async {
      final candidate = buildEvent(
        id: 'lone',
        eventDate: base,
      ...
      expect(
        ...
        EventRelationshipType.unknownRelationship,
      );
  ...
  test(
    'multiple candidate counterpart legs never arbitrarily pick one',
    () async {
      final candidate = buildEvent(
        id: 'debit2',
        eventDate: base,
  ...
  test(
    'only pairs with another own-account-transfer-flagged event, never an ordinary one',
    () async {
      final candidate = buildEvent(
        id: 'debit3',
        eventDate: base,
