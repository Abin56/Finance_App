...
void main() {
  ...
  test(
    '2. A payment request must never link as a successful payment',
    () async {
  ...
  test(
    '3. A failed transaction must never link as a successful transaction',
    () async {
      final failed = buildEvent(
        id: 'f',
        eventDate: base,
        amount: 750,
        referenceNumber: 'UTR1',
        moneyMovement: false,
        transactionStatus: TransactionStatus.failed,
      );
      final pending = buildEvent(
        id: 'p',
        eventDate: base.subtract(const Duration(minutes: 10)),
        amount: 750,
        referenceNumber: 'UTR1',
        moneyMovement: false,
  ...
  test('4. A refund must never become a duplicate', () async {
    final refund = buildEvent(
      id: 'ref',
      eventDate: base.add(const Duration(hours: 1)),
      amount: 1500,
      merchant: 'Amazon',
  ...
  test(
    '6-7. An own-account transfer must never become income or an expense',
    () async {
      ...
      final unrelated = buildEvent(
        id: 'unrelated',
        eventDate: base.add(const Duration(minutes: 1)),
        amount: 300,
        merchant: 'Swiggy',
        moneyMovement: true,
        transactionStatus: TransactionStatus.success,
  ...
  test('9. Same merchant must not imply same transaction', () async {
    ...
    final b = buildEvent(
      id: 'b2',
      eventDate: base.add(const Duration(minutes: 5)),
      amount: 850,
      merchant: 'Swiggy',
      moneyMovement: true,
      transactionStatus: TransactionStatus.success,
  ...
  test('11. Different reference IDs must not be merged', () async {
    ...
    final b = buildEvent(
      id: 'b4',
      eventDate: base.add(const Duration(days: 10)),
      amount: 999,
    ...
    expect(
      ...
      isFalse,
      reason:
          'The two different reference numbers must never themselves register as a matched signal.',
    );
  });
  ...
  test('12. Ambiguous candidates must remain ambiguous', () async {
    ...
    final b = buildEvent(
      id: 'ambB',
      eventDate: base.add(const Duration(minutes: 3)),
      amount: 500,
      merchant: 'Swiggy',
      moneyMovement: true,
      transactionStatus: TransactionStatus.success,
  ...
  test(
    '13. A later transaction must not overwrite an earlier unrelated transaction',
    () async {
      ...
      final later = buildEvent(
        id: 'later',
        eventDate: base.add(const Duration(days: 5)),
        amount: 9999,
        merchant: 'Unrelated Store',
        moneyMovement: true,
        transactionStatus: TransactionStatus.success,
      );
  ...
  test(
    '14. Multiple SMS notifications for one event must not create multiple financial events',
    () async {
  ...
  test(
    '15. Deleting one relationship edge must never destroy the underlying events',
    () async {
      final repo = InMemoryEventRelationshipRepository();
      final e1 = buildEvent(
        id: 'ev1',
