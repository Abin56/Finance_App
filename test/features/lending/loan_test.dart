...
void main() {
  group('Loan.fromFirestore / toFirestore', () {
    test(
      'round-trips every field including installment fields and interest map',
      () async {
  ...
  group('Loan.direction backward compatibility', () {
    test(
      'a Firestore doc with no direction field defaults to given on read',
      () async {
        final firestore = FakeFirebaseFirestore();
  ...
  group('Loan.category backward compatibility', () {
    test(
      'a Firestore doc with no category field defaults to personal, keeping its existing personId',
      () async {
        final firestore = FakeFirebaseFirestore();
    ...
    test(
      'round-trips an institutional loan with null personId and all institution fields',
      () async {
        final firestore = FakeFirebaseFirestore();
        final collection = firestore
            .collection('loans')
  ...
  group('Loan.payerPersonId', () {
    test(
      'defaults to null when not set, and a Firestore doc with no payerPersonId field round-trips as null',
      () async {
        final firestore = FakeFirebaseFirestore();
        final collection = firestore
            .collection('loans')
    ...
    test(
      'round-trips an explicit payerPersonId, distinct from personId — covers "I took a bank loan, a friend pays it"',
      () async {
        final firestore = FakeFirebaseFirestore();
        final collection = firestore
            .collection('loans')
  ...
  group('Loan.statusGiven', () {
    ...
    test(
      'returns overdue when any installment is overdue and loan is not closed',
      () {
    ...
    test(
      'returns active when no installment is overdue and loan is not closed',
      () {
        final loan = buildLoan();
        final installments = [
