...
void main() {
  ...
  group('LoanRepository.createLoan — validation', () {
    ...
    test(
      'installment without installmentFrequency/installmentCount throws',
      () async {
        await expectLater(
          repository.createLoan(
            personId: 'p1',
  ...
  group('LoanRepository.createLoan — category', () {
    ...
    test(
      'institutional loan persists with personId null and institution fields set, ignoring any personId passed',
      () async {
        final loan = await repository.createLoan(
          category: LoanCategory.institutional,
          personId:
              'p1', // must be ignored — an institutional loan is never person-linked
    ...
    test(
      'personal loan (default category) persists exactly as before, with no institution fields set',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 100,
  ...
  group('LoanRepository.createLoan — no interest', () {
    test(
      'oneTime loan creates a schedule with a single installment, no principal/interest split',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 1000,
    ...
    test(
      'installment loan creates N even-split installments with no interest',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 1200,
  ...
  group('LoanRepository.createLoan — with interest', () {
    test(
      'flat interest: schedule.totalAmount == loanAmount + totalInterest',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 1000,
        ...
        final totalInterest = installments.fold(
          0.0,
          (sum, i) => sum + i.interestPortion!,
        );
    ...
    test(
      'reducing balance interest: per-installment principal/interest sums correctly',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 100000,
        ...
        expect(
          installments.last.interestPortion,
          lessThan(installments.first.interestPortion!),
        );
      },
    );

    test(
      'one-time loan with interest still creates a single installment',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 1000,
    ...
    test(
      'monthly-frequency interest loan is unaffected by the weekly-normalization fix (no regression)',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 100000,
  ...
  group('LoanRepository.editLoan', () {
    test('records an audit entry per changed field', () async {
      ...
      expect(
        loan.editHistory.map((e) => e.field),
        containsAll(['name', 'notes']),
      );
    });

    test(
      'rejects changing loanAmount once a payment has been recorded',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 1000,
  ...
  group('Loan.statusGiven', () {
    test(
      'returns closed when isClosed is true regardless of installment state',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 1000,
    ...
    test(
      'returns overdue when the linked installment is overdue and loan is not closed',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 1000,
    ...
    test(
      'returns active when the installment is upcoming and loan is not closed',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 1000,
  ...
  group('Soft-delete / restore', () {
    test(
      'soft-deleting a loan does not reverse installment payments; restoring brings it back',
      () async {
        final loan = await repository.createLoan(
          personId: 'p1',
          loanAmount: 1000,
        ...
        await installmentRepositoryFor(
          loan.scheduleId,
        ).applyPayment(installment, 400);
