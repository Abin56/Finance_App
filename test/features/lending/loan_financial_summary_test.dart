...
void main() {
  group('LoanFinancialSummary.from', () {
    test('example 1: single one-time installment, partial payment', () {
      ...
      final summary = LoanFinancialSummary.from(
    ...
    test(
      'interest-bearing loan: partial payment uses proportional allocation',
      () {
        ...
        final summary = LoanFinancialSummary.from(
          installments: installments,
          originalPrincipal: 900,
        );

        // The paid amount is allocated using the installment's 90/10 split.
        expect(summary.interestPaid, closeTo(5, 0.001));
        expect(summary.principalPaid, closeTo(45, 0.001));
        expect(summary.principalRemaining, closeTo(855, 0.001));
        ...
        final summaryFull = LoanFinancialSummary.from(
          installments: installmentsFull,
          originalPrincipal: 900,
        );
        // The same proportional allocation applies to larger partial payments.
        expect(summaryFull.interestPaid, closeTo(30, 0.001));
        expect(summaryFull.principalPaid, closeTo(270, 0.001));
      },
    );

    test(
      'overdue installment contributes to overdueInstallments/overdueAmount',
      () {
        final installments = [
          _installment(
            id: 'i1',
    ...
    test(
      'a partially-paid overdue installment is never double-counted as overdue',
      () {
        final installments = [
          _installment(
            id: 'i1',
    ...
    test(
      'skipped installments are excluded from next/remaining but still count toward totalPaid',
      () {
        final installments = [
          _installment(
            id: 'i1',
    ...
    test(
      'closed/fully-settled loan with no remaining installments has null nextInstallment',
      () {
        final installments = [
          _installment(
            id: 'i1',
    ...
    test(
      'one-time loan: single installment behaves like an installment loan of length 1',
      () {
        final installments = [
          _installment(
            id: 'i1',
    ...
    test(
      'rounding: fractional amounts sum without drifting due to clamping',
      () {
        final installments = [
          _installment(
            id: 'i1',
    ...
    test(
      'outstanding never goes negative even if amountPaid overshoots amountDue',
      () {
        final installments = [
          _installment(
            id: 'i1',
    ...
    test(
      'progress is the paid fraction of scheduled payable, clamped to [0,1]',
      () {
        final installments = [
          _installment(
            id: 'i1',
