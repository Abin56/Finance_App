...
void main() {
  ...
  List<PersonTimelineEntry> buildCyclable(List<PersonTimelineEntry> all) {
    return all
        .where(
          (e) =>
              e.category == PersonTimelineCategory.assignedExpense ||
              e.category == PersonTimelineCategory.splitExpense,
        )
        .toList();
  }
  ...
  group('PersonTimelineCycleItem + CycleEngine.classifyForCarryForward', () {
    test('a previous-cycle entry that is still Pending carries forward', () {
      ...
      final entries = PersonTimelineBuilder.build(
        ...
        installmentByTransactionRef:
            {}, // no installment -> null status, treated as not-settled
      );
      final cyclable = buildCyclable(entries);
      final items = cyclable.map(PersonTimelineCycleItem.new).toList();

      final result = CycleEngine.classifyForCarryForward(
        items,
        personCycleAnchor,
    ...
    test(
      'a previous-cycle entry that is fully settled (Completed) does not carry forward',
      () {
        final entries = PersonTimelineBuilder.build(
          ledgerEntries: [giveEntry(id: 'l1', date: DateTime(2026, 6, 20))],
          loans: const [],
          participantCountByTransactionRef: {'txn1': 1},
          installmentByTransactionRef: {
            'txn1': installmentWithStatus(InstallmentStatus.paid),
          },
        );
        final cyclable = buildCyclable(entries);
        final items = cyclable.map(PersonTimelineCycleItem.new).toList();
    ...
    test('a previous-cycle entry that is Partial still carries forward', () {
      final entries = PersonTimelineBuilder.build(
        ...
        participantCountByTransactionRef: {'txn1': 1},
        installmentByTransactionRef: {
          'txn1': installmentWithStatus(InstallmentStatus.partiallyPaid),
        },
      );
      final cyclable = buildCyclable(entries);
      final items = cyclable.map(PersonTimelineCycleItem.new).toList();
    ...
    test(
      'a soft-deleted previous-cycle entry never appears (excluded before cycle classification)',
      () {
        final deleted = giveEntry(id: 'l1', date: DateTime(2026, 6, 20))
          ..deletedAt = DateTime(2026, 6, 21);
        final entries = PersonTimelineBuilder.build(
    ...
    test(
      'lending/adjustment/reference entries are excluded from cycle classification entirely',
      () {
    ...
    test(
      'a partially-paid previous-cycle entry carries forward with its real remaining amount, not the full total',
      () {
        // Owes $1000, pays $400 -> remaining should be $600, not the full $1000.
        final installment = _installment(amountDue: 1000, amountPaid: 400);
        final entries = PersonTimelineBuilder.build(
          ledgerEntries: [
            giveEntry(id: 'l1', date: DateTime(2026, 6, 20), amount: 1000),
          ],
          loans: const [],
          participantCountByTransactionRef: {'txn1': 1},
    ...
    test(
      'an installment unpaid for several cycles keeps carrying forward on every later now, unmutated',
      () {
        ...
        final entries = PersonTimelineBuilder.build(
          ledgerEntries: [
            giveEntry(id: 'l1', date: DateTime(2026, 1, 20), amount: 500),
          ],
          loans: const [],
          participantCountByTransactionRef: {'txn1': 1},
          installmentByTransactionRef: {'txn1': installment},
        ...
        for (final now in nowValues) {
          ...
          expect(result.previousCyclePending.map((i) => i.entry.id), [
            'l1',
          ], reason: 'now=$now');
        }
    ...
    test(
      'a future-cycle entry is surfaced separately and never in current/previousCyclePending',
      () {
        final entries = PersonTimelineBuilder.build(
          ...
          participantCountByTransactionRef: {'txn1': 1},
          installmentByTransactionRef: {
            'txn1': installmentWithStatus(InstallmentStatus.upcoming),
          },
        );
        final cyclable = buildCyclable(entries);
        final items = cyclable.map(PersonTimelineCycleItem.new).toList();
