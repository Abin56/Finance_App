...
void main() {
  ...
  Future<void> pumpHeader(
    ...
    return tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: PersonStatementHeader(person: p, entries: entries),
        ),
      ),
    );
  }
  ...
  testWidgets(
    'sums money given/received from manual ledger-entry-labeled titles, not just loan titles',
    (tester) async {
      final entries = [
      ...
      expect(
        _amountNear(tester, 'Total money given'),
        '₹1,500.00',
        reason: '1000 (gave) + 500 (repaid)',
      );
      expect(
        _amountNear(tester, 'Total money received'),
        '₹500.00',
        reason: '300 (borrowed) + 200 (receivedBack)',
      );
    },
  );

  testWidgets(
    'includes split/assigned-expense and adjustment entries in the given/received totals',
    (tester) async {
      final entries = [
        entry(
          id: 'e1',
        ...
        entry(
          ...
          signedAmount: -100,
          category: PersonTimelineCategory.other,
        ),
      ];
      ...
      expect(
        _amountNear(tester, 'Total money given'),
        '₹500.00',
        reason: '200 (split) + 300 (assigned)',
      );
      expect(
        _amountNear(tester, 'Total money received'),
        '₹100.00',
        reason: '100 (adjustment)',
      );
    },
  );

  testWidgets('shows ₹0.00 for both totals when there are no entries', (
    tester,
  ) async {
