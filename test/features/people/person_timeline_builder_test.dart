...
/// suite runs.
Installment _installmentWithStatus(
  InstallmentStatus status, {
  String scheduleId = 'sched1',
  double amountDue = 800,
}) {
  switch (status) {
...
void main() {
  group('PersonTimelineBuilder.build merge ordering', () {
    test(
      'merges ledger entries and loan events into one chronological list',
      () {
        final ledgerEntries = [
          LedgerEntry(
            id: 'l1',
  ...
  group('PersonTimelineBuilder.runningBalances', () {
    test(
      'matches a hand-computed running balance across mixed entry types',
      () {
        final ledgerEntries = [
          LedgerEntry(
            id: 'l1',
        ...
        final balances = PersonTimelineBuilder.runningBalances(
          openingBalance: 0,
  ...
  group('PersonTimelineBuilder category tagging', () {
    ...
    test(
      'plain gave/borrowed/receivedBack/repaid entries are categorized as lending',
      () {
        final entry = LedgerEntry(
          id: 'l1',
          personId: 'p1',
    ...
    test(
      'a split-note entry with a single participant is categorized as assignedExpense',
      () {
        final entry = LedgerEntry(
          id: 'l1',
          personId: 'p1',
    ...
    test(
      'a split-note entry with multiple participants is categorized as splitExpense',
      () {
        final entry = LedgerEntry(
          id: 'l1',
          personId: 'p1',
    ...
    test('loan creation and loan payments are categorized as lending', () {
      ...
      expect(
        result.every((e) => e.category == PersonTimelineCategory.lending),
        isTrue,
      );
    });

    test(
      'a given loan posts a positive signed amount and a payment reduces it',
      () {
        final loan = _loan(
          id: 'loan1',
          loanAmount: 200,
        ...
        final paymentEntry = result.singleWhere(
          (e) => e.id == 'loan-payment-pay1',
        );
    ...
    test(
      'a taken loan posts a negative signed amount and a payment reduces the debt (moves it toward zero)',
      () {
        final loan = _loan(
          id: 'loan2',
          loanAmount: 200,
  ...
  group('PersonTimelineBuilder split expense status', () {
    ...
    test(
      'a split "gave" entry shows Pending when its installment is upcoming',
      () {
        final result = PersonTimelineBuilder.build(
          ...
          installmentByTransactionRef: {
            'txn1': _installmentWithStatus(InstallmentStatus.upcoming),
          },
        );
    ...
    test(
      'a split "gave" entry shows Partial when its installment is partially paid',
      () {
        final result = PersonTimelineBuilder.build(
          ledgerEntries: [giveEntry()],
          loans: const [],
          participantCountByTransactionRef: {'txn1': 2},
          installmentByTransactionRef: {
            'txn1': _installmentWithStatus(InstallmentStatus.partiallyPaid),
          },
        );
    ...
    test(
      'a split "gave" entry shows Completed when its installment is fully paid',
      () {
        final result = PersonTimelineBuilder.build(
          ledgerEntries: [giveEntry()],
          loans: const [],
          participantCountByTransactionRef: {'txn1': 2},
          installmentByTransactionRef: {
            'txn1': _installmentWithStatus(InstallmentStatus.paid),
          },
        );
    ...
    test(
      'a split "gave" entry shows Overdue when its installment is overdue',
      () {
        final result = PersonTimelineBuilder.build(
          ledgerEntries: [giveEntry()],
          loans: const [],
          participantCountByTransactionRef: {'txn1': 2},
          installmentByTransactionRef: {
            'txn1': _installmentWithStatus(InstallmentStatus.overdue),
          },
        );
    ...
    test(
      'a split "gave" entry carries totalAmount/paidAmount from its installment',
      () {
        final installment = _installment(
          scheduleId: 'sched1',
          amountDue: 800,
    ...
    test(
      'a non-expense entry never carries totalAmount/paidAmount even if an installment is present',
      () {
        final entry = LedgerEntry(
          id: 'l1',
          personId: 'p1',
    ...
    test(
      'a split settlement ("receivedBack") entry always shows Completed',
      () {
  ...
  group('PersonTimelineBuilder — referencedTransactions', () {
    ...
    test(
      'a reference-only transaction produces a zero-amount, no-status, "reference" entry',
      () {
        final result = PersonTimelineBuilder.build(
          ledgerEntries: const [],
          loans: const [],
    ...
    test(
      'a soft-deleted referenced transaction is excluded by default, included with includeDeleted',
      () {
    ...
    test(
      'a reference entry never contributes to the pending balance total, unlike a real gave entry',
      () {
        ...
        final totalSignedAmount = result.fold(
          0.0,
          (total, e) => total + e.signedAmount,
        );
