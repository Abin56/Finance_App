...
void main() {
  ...
  group('PersonRepository.createPerson', () {
    ...
    test(
      'allows the same name with no phone or email on either record',
      () async {
        await repository.createPerson(
          name: 'Alex',
          avatarColorValue: 0xFF5B5FEF,
  ...
  group('PersonRepository.editPerson', () {
    test('does not expose a way to change openingBalance', () async {
      ...
      expect(
        ...
        300,
        reason: 'opening balance is immutable post-creation',
      );
    ...
    test('records an audit entry per changed field', () async {
      ...
      expect(
        person.editHistory.map((e) => e.field),
  ...
  group('PersonRepository.deletePersonAndLedger', () {
    ...
    test(
      'permanently deletes every active ledger entry, not just the person',
      () async {
        final person = await repository.createPerson(
          name: 'Alex',
          avatarColorValue: 0xFF5B5FEF,
        ...
        expect(
          await ledger.getAll(),
          isEmpty,
          reason: 'active ledger entries must not be orphaned',
        );
      },
    );

    test(
      'permanently deletes trashed ledger entries too, not only active ones',
      () async {
        final person = await repository.createPerson(
          name: 'Alex',
          avatarColorValue: 0xFF5B5FEF,
        ...
        expect(
          await ledger.getTrash(),
          isEmpty,
          reason: 'trashed ledger entries must not be orphaned either',
        );
      },
    );

    test(
      'is safe to call for a person with no ledger entries at all',
      () async {
        final person = await repository.createPerson(
          name: 'Alex',
          avatarColorValue: 0xFF5B5FEF,
