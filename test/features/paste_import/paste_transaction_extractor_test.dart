...
void main() {
  ...
  group(
    'PasteTransactionExtractor — format 1 (date merchant amount DR/CR per line)',
    () {
  ...
  group(
    'PasteTransactionExtractor — format 2 (blank-line separated, date/merchant/amount)',
    () {
  ...
  group(
    'PasteTransactionExtractor — format 3 (UPI dash-dated, Debit wording)',
    () {
      test('parses dash-separated dates with Debit wording', () {
        final text =
            '05-09-26 UPI-SWIGGY 420.00 Debit\n06-09-26 UBER INDIA 185.50 Debit';
        final result = PasteTransactionExtractor.extract(
          text,
          referenceDate: reference,
        );
  ...
  group(
    'PasteTransactionExtractor — format 4 (Mon dd merchant amount, columns)',
    () {
  ...
  group(
    'PasteTransactionExtractor — format 5 (blank-line separated, merchant/date/amount)',
    () {
  ...
  group('PasteTransactionExtractor — amount formats', () {
    test(
      'handles currency symbol, commas and INR marker without misreading them',
      () {
        expect(
          PasteTransactionExtractor.extract(
            '05 Sep TEST ₹1,299.00 DR',
            referenceDate: reference,
          ).first.amount,
          1299.0,
        );
    ...
    test(
      'does not misread a comma-grouped amount as a decimal (1,299 -> 1299.00, not 1.299)',
      () {
        final result = PasteTransactionExtractor.extract(
  ...
  group('PasteTransactionExtractor — debit/credit detection', () {
    test('DR maps to expense and CR maps to income', () {
      final debit = PasteTransactionExtractor.extract(
        '05 Sep TEST 420 DR',
        referenceDate: reference,
      );
      final credit = PasteTransactionExtractor.extract(
        '05 Sep TEST 500 CR',
        referenceDate: reference,
      );
  ...
  group('PasteTransactionExtractor — invalid/ambiguous input', () {
    ...
    test(
      'returns an empty list for text with no recognizable transactions',
      () {
        final result = PasteTransactionExtractor.extract(
    ...
    test(
      'flags a transaction with no confident amount as needing review rather than guessing',
      () {
        final result = PasteTransactionExtractor.extract(
          '05 Sep SWIGGY',
          referenceDate: reference,
    ...
    test(
      'partial parsing keeps successfully-parsed rows even when others need review',
      () {
        final text =
  ...
  group(
    'PasteTransactionExtractor — real-world Google Pay / PhonePe style copy text',
    () {
      test(
        'extracts the reference number from "UPI transaction ID:" wording',
        () {
          final text =
      ...
      test(
        'strips "Paid to" boilerplate and the timestamp out of the description, '
      ...
      test(
        'does not strip "to"/"from" when they are not a leading connector',
        () {
  ...
  group('PasteTransactionExtractor — date/amount adjacency regression', () {
    test(
      'does not misread an amount immediately after the date as a 3-digit year',
      () {
  ...
  test('extract returns an empty list for blank input', () {
    expect(
      PasteTransactionExtractor.extract('', referenceDate: reference),
      isEmpty,
    );
  });
}
