...
List<DetectedTransaction> _selectForImport(List<DetectedTransaction> detected) {
  final candidates = detected.where((d) => d.isSelected && !d.isImported).toList();
  return candidates.where((d) => !d.isDuplicate || d.duplicateAcknowledged).toList();
}
...
void main() {
  ...
  group('Paste Import — end-to-end from pasted text to real transactions', () {
    test('a pasted transaction list is parsed and imported through the shared repository', () async {
    ...
    test('a pasted transaction matching an existing one is flagged as a possible duplicate '
        'and skipped rather than silently re-imported', () async {
      await transactionRepository.createTransaction(
      ...
      final saved = await transactionRepository.getAll();
      expect(saved, hasLength(2)); // the pre-existing Swiggy + Uber, not a second Swiggy
    });

    test('an explicit "import anyway" override lets a duplicate through', () async {
      await transactionRepository.createTransaction(
        type: TransactionType.expense,
        amount: 420,
        dateTime: DateTime(2026, 9, 5),
    ...
    test('a row that could not be confidently parsed is excluded from import until edited', () async {
      final detected = PasteTransactionExtractor.extract(
    ...
    test('retrying after a successful import never creates the transaction twice', () async {
      final detected = PasteTransactionExtractor.extract(
        '05 Sep SWIGGY 420 DR',
        referenceDate: reference,
      );
