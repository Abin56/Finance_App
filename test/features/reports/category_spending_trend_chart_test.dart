...
void main() {
  ...
  double chartTotal(WidgetTester tester) {
    ...
    return lineChart.data.lineBarsData.single.spots.fold(
      0.0,
      (sum, s) => sum + s.y,
    );
  }
  ...
  testWidgets(
    'chart total matches the header total for plain same-month transactions',
    (tester) async {
  ...
  testWidgets(
    'a transaction reassigned to this month via accountingMonth appears in the chart',
    (tester) async {
      final periodStart = DateTime(2026, 7, 1);
      final periodEnd = DateTime(2026, 7, 31);
      ...
      expect(
        chartTotal(tester),
        650,
        reason: 'must land on a day within the range, not silently vanish',
      );
    },
  );

  testWidgets(
    'non-month-granular period buckets by the real date, unaffected by accountingMonth',
    (tester) async {
      final periodStart = DateTime(2026, 7, 1);
