...
void main() {
  group('ReportsPeriod.financialYear.rangeFor', () {
    test('January start month collapses to the calendar year', () {
      ...
      final range = ReportsPeriod.financialYear.rangeFor(
        now,
    ...
    test(
      'April start month, date after April, runs Apr this year to Mar next year',
      () {
        final now = DateTime(2026, 7, 7);
        final range = ReportsPeriod.financialYear.rangeFor(
          now,
    ...
    test(
      'April start month, date before April (January), runs previous Apr to this Mar',
      () {
  ...
  group('ReportsPeriodX.reportDateFor', () {
    test(
      'thisMonth reads effectiveMonth (accounting month) when set, not the real date',
      () {
        final t = _transaction(
          dateTime: DateTime(2026, 6, 15),
          accountingMonth: DateTime(2026, 7),
        );
    ...
    test(
      'lastMonth also reads effectiveMonth — both month-granular periods behave the same',
      () {
        final t = _transaction(
          dateTime: DateTime(2026, 6, 15),
          accountingMonth: DateTime(2026, 7),
    ...
    test(
      'falls back to dateTime when no accounting month is set, for a month-granular period',
      () {
    ...
    test(
      'non-month-granular periods (today/week/year/financialYear) always use the real dateTime',
      () {
        final t = _transaction(
          dateTime: DateTime(2026, 6, 15, 9, 30),
          accountingMonth: DateTime(2026, 7),
        );

        expect(
          ReportsPeriod.today.reportDateFor(t),
          DateTime(2026, 6, 15, 9, 30),
        );
        expect(
          ReportsPeriod.thisWeek.reportDateFor(t),
          DateTime(2026, 6, 15, 9, 30),
        );
        expect(
          ReportsPeriod.thisYear.reportDateFor(t),
          DateTime(2026, 6, 15, 9, 30),
        );
        expect(
          ReportsPeriod.financialYear.reportDateFor(t),
          DateTime(2026, 6, 15, 9, 30),
        );
      },
    );
  ...
  group('Month-boundary regression — rangeFor + reportDateFor composed together', () {
    test(
      'a transaction dated the last second of last month but reassigned to this month is included',
      () {
        ...
        expect(
          range.contains(ReportsPeriod.thisMonth.reportDateFor(t)),
          isTrue,
        );
      },
    );
    ...
    test(
      'a transaction dated the first second of this month but reassigned to last month is excluded',
      () {
        final now = DateTime(2026, 7, 15);
        final range = ReportsPeriod.thisMonth.rangeFor(now);
        final t = _transaction(
          dateTime: DateTime(2026, 7, 1, 0, 0, 1),
    ...
    test(
      'with no accounting month, a transaction exactly on the month boundary follows its real date',
      () {
        final now = DateTime(2026, 7, 15);
        final range = ReportsPeriod.thisMonth.rangeFor(now);

        final lastSecondOfJune = _transaction(
          dateTime: DateTime(2026, 6, 30, 23, 59, 59),
        );
        final firstSecondOfJuly = _transaction(
          dateTime: DateTime(2026, 7, 1, 0, 0, 0),
        );

        expect(
          range.contains(
            ReportsPeriod.thisMonth.reportDateFor(lastSecondOfJune),
          ),
          isFalse,
        );
        expect(
          range.contains(
            ReportsPeriod.thisMonth.reportDateFor(firstSecondOfJuly),
          ),
          isTrue,
        );
      },
