...
void main() {
  ...
  group('TransactionRepository balance sync', () {
    ...
    test(
      'createTransaction defaults source to null for a normal (manual) create',
      () async {
        final account = await seedAccount();
    ...
    test(
      'editTransaction applies the net delta when the account is unchanged',
      () async {
        final account = await seedAccount();
    ...
    test(
      'editTransaction moves the balance effect when the account changes',
      () async {
        ...
        expect(
          updatedA!.currentBalance,
          1000,
          reason: 'old account balance fully reversed',
        );
        expect(
          updatedB!.currentBalance,
          900,
          reason: 'new account balance fully applied',
        );
      },
    );
    ...
    test(
      'permanentlyDeleteTransaction does not change the balance again',
      () async {
        final account = await seedAccount();
        final transaction = await transactionRepository.createTransaction(
          type: TransactionType.income,
  ...
  group('TransactionRepository — excludeFromCalculations', () {
    test(
      'createTransaction with excludeFromCalculations=true does not change the balance',
      () async {
        final account = await seedAccount();
        ...
        expect(
          ...
          1000,
          reason: 'excluded expense must not affect balance',
        );
      },
    );

    test(
      'createTransaction with excludeFromCalculations=true does not change balance for income either',
      () async {
        final account = await seedAccount();
        ...
        expect(
          updated!.currentBalance,
          1000,
          reason: 'excluded income must not affect balance',
        );
      },
    );

    test(
      'editTransaction toggling excludeFromCalculations to true reverses the balance effect',
      () async {
        final account = await seedAccount();
        final transaction = await transactionRepository.createTransaction(
          type: TransactionType.expense,
        ...
        expect(
          updated!.currentBalance,
          1000,
          reason: 'toggling exclude on must reverse the prior balance effect',
        );
      },
    );

    test(
      'editTransaction toggling excludeFromCalculations back to false re-applies the balance effect',
      () async {
        final account = await seedAccount();
        final transaction = await transactionRepository.createTransaction(
          type: TransactionType.expense,
        ...
        expect(
          updated!.currentBalance,
          700,
          reason: 'toggling exclude off must apply the balance effect',
        );
      },
    );

    test(
      'editTransaction amount change while excluded does not affect balance',
      () async {
        final account = await seedAccount();
        final transaction = await transactionRepository.createTransaction(
          type: TransactionType.expense,
        ...
        expect(
          ...
          1000,
          reason:
              'amount changes on an excluded transaction stay balance-neutral',
        );
      },
    );
  });
  ...
  group('TransactionRepository — accountingMonth', () {
    ...
    test(
      'editTransaction clears accountingMonth back to "same as transaction date"',
      () async {
        final account = await seedAccount();
        final transaction = await transactionRepository.createTransaction(
          type: TransactionType.expense,
    ...
    test(
      'accountingMonth never affects balance — it is a reporting-only concern',
      () async {
        final account = await seedAccount();
        ...
        expect(
          ...
          600,
          reason:
              'balance reflects the real transaction, regardless of its accounting month',
        );
      },
    );
  });
