...
void main() {
  group('Emi.fromFirestore / toFirestore', () {
    test(
      'round-trips every field including nullable lenderName/categoryId/interest',
      () async {
    ...
    test(
      'defaults linkedCreditCardId and dueDayOfMonth to null for a document written before these fields existed',
      () async {
        final firestore = FakeFirebaseFirestore();
        final collection = firestore
            .collection('emis')
    ...
    test(
      'round-trips an EMI with no interest and no lender/category',
      () async {
        final firestore = FakeFirebaseFirestore();
        final collection = firestore
            .collection('emis')
    ...
    test(
      'parses a pre-existing document that has none of the new loan-management fields',
      () async {
        final firestore = FakeFirebaseFirestore();
        final collection = firestore
            .collection('emis')
  ...
  group('Emi.statusGiven', () {
    ...
    test('returns active when no installment is overdue and not closed', () {
      final emi = buildEmi();
      final installments = [
        _installment(dueDate: DateTime.now().add(const Duration(days: 10))),
      ];
    ...
    test(
      'returns completed when every installment is fully paid and not closed',
      () {
        final emi = buildEmi();
        final installments = [
          _installment(
    ...
    test(
      'returns active, not completed, when an installment is still partially unpaid',
      () {
        final emi = buildEmi();
        final installments = [
          _installment(
    ...
    test(
      'returns active, not completed, for an EMI with no installments yet',
      () {
        final emi = buildEmi();
  ...
  group('emiInstallmentStatusLabel', () {
    test(
      'relabels upcoming as Unpaid when due date is in the current month',
      () {
        ...
        expect(
          emiInstallmentStatusLabel(InstallmentStatus.upcoming, dueThisMonth),
          'Unpaid',
        );
      },
    );
    ...
    test('keeps upcoming as Upcoming when due date is in a future month', () {
      ...
      expect(
        emiInstallmentStatusLabel(InstallmentStatus.upcoming, dueNextMonth),
        'Upcoming',
      );
    });
    ...
    test('overdue stays Missed Payment regardless of month', () {
      ...
      expect(
        emiInstallmentStatusLabel(InstallmentStatus.overdue, dueThisMonth),
        'Missed Payment',
      );
    });
    ...
    test('paid stays Paid', () {
      ...
      expect(
        emiInstallmentStatusLabel(InstallmentStatus.paid, dueThisMonth),
        'Paid',
      );
    });
  });
