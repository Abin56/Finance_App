...
void main() {
  group('BillRecurrence.nextDueDate', () {
    ...
    test('daily adds 1 day', () {
      expect(
        BillRecurrence.daily.nextDueDate(DateTime(2026, 3, 15)),
        DateTime(2026, 3, 16),
      );
    });
    ...
    test('weekly adds 7 days', () {
      expect(
        BillRecurrence.weekly.nextDueDate(DateTime(2026, 3, 15)),
        DateTime(2026, 3, 22),
      );
    });
    ...
    test('monthly adds a calendar month', () {
      expect(
        BillRecurrence.monthly.nextDueDate(DateTime(2026, 3, 15)),
        DateTime(2026, 4, 15),
      );
    });
    ...
    test('monthly clamps to the last day of a shorter target month', () {
      ...
      expect(
        BillRecurrence.monthly.nextDueDate(DateTime(2026, 1, 31)),
        DateTime(2026, 2, 28),
      );
    });
    ...
    test('monthly clamps correctly across a leap year February', () {
      ...
      expect(
        BillRecurrence.monthly.nextDueDate(DateTime(2028, 1, 31)),
        DateTime(2028, 2, 29),
      );
    });
    ...
    test('monthly rolls over into the next year at December', () {
      expect(
        BillRecurrence.monthly.nextDueDate(DateTime(2026, 12, 15)),
        DateTime(2027, 1, 15),
      );
    });
    ...
    test('yearly adds 12 months, preserving day/month', () {
      expect(
        BillRecurrence.yearly.nextDueDate(DateTime(2026, 3, 15)),
        DateTime(2027, 3, 15),
      );
    });

    test(
      'yearly clamps Feb 29 on a leap year to Feb 28 the next (non-leap) year',
      () {
        expect(
          BillRecurrence.yearly.nextDueDate(DateTime(2028, 2, 29)),
          DateTime(2029, 2, 28),
        );
      },
    );
    ...
    test('custom adds the given number of days', () {
      expect(
        BillRecurrence.custom.nextDueDate(
          DateTime(2026, 3, 15),
          customDays: 10,
        ),
        DateTime(2026, 3, 25),
    ...
    test('custom defaults to 1 day when customDays is not provided', () {
      expect(
        BillRecurrence.custom.nextDueDate(DateTime(2026, 3, 15)),
        DateTime(2026, 3, 16),
      );
    });
  });
