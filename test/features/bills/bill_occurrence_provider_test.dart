...
void main() {
  ...
  Future<String> createBill({
    ...
    BillRecurrence recurrence = BillRecurrence.oneTime,
  }) async {
  ...
  test(
    'a brand-new bill materializes exactly one current occurrence',
    () async {
  ...
  test('materializing twice does not create a second occurrence', () async {
    ...
    final occurrences = container
        .read(billOccurrencesStreamProvider(billId))
        .value;
    await materialize(billId);
    final occurrencesAfterSecondCall = container
        .read(billOccurrencesStreamProvider(billId))
        .value;
  ...
  test(
    'an unpaid occurrence due before the current cycle carries forward as pending',
    () async {
  ...
  test(
    'a fully paid occurrence due before the current cycle does not carry forward',
    () async {
      final billId = await createBill(nextDueDate: closedPeriodDueDate);
      await materialize(billId);
      final occurrence = container.read(currentBillOccurrenceProvider(billId))!;

      final occurrenceRepo = container.read(
        billOccurrenceRepositoryProvider(billId),
      );
