...
void main() {
  ...
  group('BillOccurrenceCycleItem', () {
    ...
    test(
      'sourceId points back at the owning bill, not the occurrence itself',
      () {
        final item = BillOccurrenceCycleItem(
          occurrence(dueDate: DateTime(2026, 7, 17)),
        );
  ...
  group('BillOccurrenceCycleItem + CycleEngine.classifyForCarryForward', () {
    test(
      'an unpaid previous-cycle occurrence carries forward — the case the old adapter could not represent',
      () {
        ...
        expect(result.previousCyclePending.map((i) => i.occurrence.id), [
          'prev',
        ]);
    ...
    test(
      'a partially paid previous-cycle occurrence still carries forward',
      () {
        final previous = occurrence(
          id: 'prev',
          dueDate: DateTime(2026, 6, 20),
    ...
    test(
      'a current-cycle occurrence always shows regardless of settled state',
      () {
    ...
    test(
      'a genuinely distinct previous occurrence and current occurrence coexist independently',
      () {
        ...
        final items = [
          previous,
          current,
        ].map(BillOccurrenceCycleItem.new).toList();
