...
void main() {
  ...
  BillOccurrenceRepository occurrenceRepositoryFor(
    String billId,
  ...
  setUp(() {
    ...
    final accountDeletionRepositories = AccountDeletionRepositories(
      ...
      billOccurrenceRepositoryFor: (billId) =>
          occurrenceRepositoryFor(billId, billRepository),
  ...
  group('previewCreditCardDeletionImpact', () {
    test(
      'reports linked EMI/statement counts and flags an orphaned shared limit',
      () async {
        final sharedLimit = await sharedCreditLimitRepository.createSharedLimit(
          name: 'Facility',
          creditLimit: 100000,
        );
    ...
    test(
      'does not flag the shared limit when a sibling card still uses it',
      () async {
        final sharedLimit = await sharedCreditLimitRepository.createSharedLimit(
          name: 'Facility',
          creditLimit: 100000,
  ...
  group('permanentlyDeleteCreditCardAndHistory', () {
    test(
      'cascades the linked EMI, statement + its payments, an orphaned shared limit, and both root docs',
      () async {
        final sharedLimit = await sharedCreditLimitRepository.createSharedLimit(
          name: 'Facility',
          creditLimit: 100000,
        ...
        expect(
          await sharedCreditLimitRepository.getByKey(sharedLimit.id),
          isNull,
        );
    ...
    test(
      'leaves the shared limit alone when a sibling card still uses it',
      () async {
        final sharedLimit = await sharedCreditLimitRepository.createSharedLimit(
          name: 'Facility',
          creditLimit: 100000,
