...
void main() {
      ...
      ProviderScope(
        overrides: [
          ...
          sharedCreditLimitsStreamProvider.overrideWith(
            (ref) => Stream.value([sharedLimit]),
          ),
          for (final card in cards)
            statementsStreamProvider(
              card.id,
            ).overrideWith((ref) => Stream.value(const [])),
          sharedCreditLimitStandingProvider(
            sharedLimit.id,
          ).overrideWithValue(facilityStanding),
          for (final entry in perCardStanding.entries)
            creditCardStandingProvider(
              entry.key,
  ...
  testWidgets(
    'two-card facility renders the hero summary and linked cards row without overflow',
    (tester) async {
      ...
      expect(
        buttonRect.right,
        lessThanOrEqualTo(_smallPhone.width),
        reason: 'button overflows screen width',
      );
      ...
      expect(
        find.textContaining(
          'This card will use the same approved credit limit',
        ),
        findsOneWidget,
      );
  ...
  testWidgets(
    'single-card facility still shows its physical-card count and add-another affordance',
    (tester) async {
      final sharedLimit = SharedCreditLimit(
        id: 'shared1',
        name: 'HDFC',
