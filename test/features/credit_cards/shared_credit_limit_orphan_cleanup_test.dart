...
void main() {
  ...
  test(
    'going standalone as the sole remaining card trashes the now-empty SharedCreditLimit',
    () async {
      ...
      expect(
        ...
        isEmpty,
        reason:
            'the orphaned SharedCreditLimit must be trashed once its last card leaves',
      );
    },
  );

  test(
    'going standalone on one of two cards keeps the facility active for the remaining sibling',
    () async {
      ...
      expect(
        ...
        hasLength(1),
        reason:
            'the facility still has a card (RuPay) attached, so it must stay active',
      );
    },
  );

  test(
    'moving a card to a different shared limit trashes the facility it left',
    () async {
      final accountVisa = await createCardAccount('Visa');

      final sharedLimits = container.read(sharedCreditLimitRepositoryProvider);
      final oldSharedLimit = await sharedLimits.createSharedLimit(
        name: 'Old Facility',
        creditLimit: 100000,
      );
      final newSharedLimit = await sharedLimits.createSharedLimit(
        name: 'New Facility',
        creditLimit: 150000,
      );
  ...
  test(
    'a CreditCardRepository with no injected SharedCreditLimitRepository still updates without error',
    () async {
      final firestore = FakeFirebaseFirestore();
