...
void main() {
  ...
  group('CreditCardRepository.createCard — loan-management-style fields', () {
    test(
      'persists network/last-4/fees/reward notes passed at creation',
      () async {
    ...
    test(
      'rejects a last-4-digits value that is not exactly 4 digits',
      () async {
        await expectLater(
    ...
    test(
      'rejects a last-4-digits value containing non-digit characters',
      () async {
        await expectLater(
          repository.createCard(
            accountId: 'a1',
  ...
  group('CreditCardRepository.editCard — loan-management-style fields', () {
    test(
      'updates network/last-4/fees/reward notes/auto debit account',
      () async {
        final card = await repository.createCard(
          accountId: 'a1',
          statementDay: 17,
