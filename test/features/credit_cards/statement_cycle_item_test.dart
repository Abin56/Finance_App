...
void main() {
  ...
  group('StatementCycleItem', () {
    ...
    test(
      'isSettled is false while any amount remains, including partial payment',
      () {
        final item = StatementCycleItem(
          statement(
            totalAmount: 1000,
    ...
    test(
      'isOverdue matches Statement.status == overdue for an unpaid past-due statement',
      () {
        final s = statement(
          totalAmount: 1000,
          amountPaid: 0,
    ...
    test(
      'isOverdue is false for a fully paid statement even if its due date is in the past',
      () {
