...
void main() {
  Future<void> pump(
    ...
    return tester.pumpWidget(
      MaterialApp(
        ...
        home: Scaffold(
          body: TransactionKindBadge(kind: kind, compact: compact),
        ),
      ),
    );
  }
  ...
  testWidgets('applies the kind\'s color to both icon and text', (
    tester,
  ) async {
    await pump(tester, TransactionKind.myExpense);

    final icon = tester.widget<Icon>(
      find.byIcon(TransactionKind.myExpense.icon),
    );
    expect(icon.color, TransactionKind.myExpense.color);

    final text = tester.widget<Text>(
      find.text(TransactionKind.myExpense.label),
    );
    expect(text.style?.color, TransactionKind.myExpense.color);
  });

  testWidgets('compact mode uses smaller padding/icon size than default', (
    tester,
  ) async {
    await pump(tester, TransactionKind.bill, compact: true);
    final compactIcon = tester.widget<Icon>(
      find.byIcon(TransactionKind.bill.icon),
    );
  ...
  testWidgets(
    'label truncates with ellipsis rather than overflowing when width is constrained',
    (tester) async {
