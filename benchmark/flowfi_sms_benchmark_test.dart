...
void main() {
  test('FlowFi SMS Intelligence Benchmark — full report (Phase 6 Foundation)', () async {
    ...
    final linkingReport = await linkingHarness.evaluateAll(linkingTestCorpus);
    ...
    w(
      'Reference number (never-invented guard): ${fieldStats.pct('referenceNumber')}',
    );
    w('');
    ...
    w(
      'Total dangerous cases evaluated across all corpora: '
    ...
    try {
      File('benchmark/last_run_output.txt').writeAsStringSync(output);
    } catch (_) {
