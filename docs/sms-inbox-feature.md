...
- **Trigger model**: manual only. `SmsInboxItemsNotifier.scan()` (`presentation/providers/sms_inbox_providers.dart`) is invoked when the user opens/refreshes the SMS Inbox screen. There is **no** background listener, no periodic scan, and it is never triggered implicitly from Dashboard or History loads.
...
- `domain/sms_dedup_key.dart` — `SmsDedupKey.compute()`: SHA-256 hash of normalized sender + timestamp + amount + (reference number or body).
...
- `presentation/widgets/`:
  - `sms_permission_gate_view.dart` — grant/settings flow when permission isn't yet granted.
...
No real-time SMS listener and no background/periodic scan exists. Scans are strictly manual and on-demand (latest 500 inbox messages), with results deduped against local sqflite state on every run.
