...
class LoanDetailScreen extends ConsumerWidget {
                      ...
                      if (isInstallment) ...[
                        if (loan.installmentFrequency != null)
                          _detailRow(
                            context,
                            'Frequency',
                            loan.installmentFrequency!.label,
                          ),
                ...
                Wrap(
                  ...
                  children: [
                    if (!isClosed &&
                      ...
                      FilledButton.icon(
                        onPressed: () => RecordLoanPaymentSheet.show(
                          context,
                    ...
                    if (!isClosed && summary.outstanding > 0)
                      OutlinedButton.icon(
                        onPressed: () => RecordLoanLumpSumSettlementSheet.show(
                          context,
                          loan,
                        ),
                    ...
                    if (!isClosed && isInstallment && summary.outstanding > 0)
                      OutlinedButton.icon(
                        onPressed: () => LoanAdjustmentSheet.show(
                          context,
                          loan: loan,
                          installments: sorted,
                        ...
                        label: const Text('Prepay Principal'),
                      ),
                    if (!isClosed && isInstallment)
                      OutlinedButton.icon(
                        onPressed: () => LoanAdjustmentSheet.show(
                          context,
                          loan: loan,
                        ...
                        label: const Text('Additional Disbursement'),
                      ),
                    OutlinedButton.icon(
                      onPressed: () => LoanFormSheet.show(context, loan: loan),
                      icon: const Icon(Icons.edit_outlined),
  ...
  Widget _buildNextPaymentCard(BuildContext context, Installment installment) {
    ...
    if (isOverdue) {
      label = 'Overdue';
      color = AppColors.error;
      final daysAgo = DateTime.now().dateOnly
          .difference(installment.dueDate.dateOnly)
          .inDays;
