...
final loanPaymentHistoryProvider = Provider.autoDispose.family<List<LoanPaymentHistoryEntry>, Loan>((ref, loan) {
  final installments = ref.watch(installmentsStreamProvider(loan.scheduleId)).value ?? const [];
  ...
  for (final raw in rawEntries) {
    ...
    entries.add(LoanPaymentHistoryEntry(
      payment: raw.payment,
      installmentSequenceNumber: raw.installment.sequenceNumber,
      status: LoanPaymentHistoryEntry.statusFor(raw.payment, raw.installment),
...
final activeLoansWithSummaryProvider = Provider<List<LoanWithSummary>>((ref) {
  final loans = ref.watch(activeLoansProvider);
  return [
    for (final loan in loans) (loan: loan, summary: ref.watch(loanFinancialSummaryProvider(loan))),
  ];
});
...
final loanDashboardMetricsProvider = Provider<LoanDashboardMetrics>((ref) {
  ...
  return LoanDashboardMetrics.from(
    loansWithSummary,
    installmentsFor: (loan) => ref.watch(installmentsStreamProvider(loan.scheduleId)).value ?? const [],
  );
});
...
final loanMonthlyRepaymentsProvider = Provider.autoDispose.family<List<MonthlyRepaymentBucket>, int>((ref, monthsAhead) {
  final loans = ref.watch(activeLoansProvider);
  final installments = <Installment>[
    for (final loan in loans) ...?ref.watch(installmentsStreamProvider(loan.scheduleId)).value,
  ];
  return LoanDashboardMetrics.monthlyRepayments(installments, monthsAhead: monthsAhead);
});
...
final loanTimelineProvider = Provider.autoDispose.family<List<LoanTimelineEntry>, Loan>((ref, loan) {
  final installments = ref.watch(installmentsStreamProvider(loan.scheduleId)).value ?? const [];
  ...
  for (final installment in installments) {
    ...
    payments.addAll(ref.watch(installmentPaymentsStreamProvider(key)).value ?? const []);
    deletedPayments.addAll(ref.watch(installmentPaymentsTrashStreamProvider(key)).value ?? const []);
  }
