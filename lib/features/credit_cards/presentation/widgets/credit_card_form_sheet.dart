...
class CreditCardFormSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CreditCardFormSheet> createState() =>
...
class _CreditCardFormSheetState extends ConsumerState<CreditCardFormSheet> {
  ...
  late final _statementDayController = TextEditingController(
    text: widget.card?.statementDay.toString() ?? '',
  );
  late final _paymentDueDayController = TextEditingController(
    text: widget.card?.paymentDueDay.toString() ?? '',
  );
  late final _creditLimitController = TextEditingController(
    text: widget.card?.creditLimit.toStringAsFixed(2) ?? '',
  );
  ...
  int _pairColorValue = AppColors.cardTintPalette[1].toARGB32();
  ...
  late final _lastFourDigitsController = TextEditingController(
    text: widget.card?.lastFourDigits ?? '',
  )..addListener(() => setState(() {}));
  late final _autoDebitAccountController = TextEditingController(
    text: widget.card?.autoDebitAccount ?? '',
  );
  late final _cardHolderNameController = TextEditingController(
  ...
  late CreditCardStatus _status =
      widget.card?.status ?? CreditCardStatus.active;
  ...
  void initState() {
    ...
    if (card != null) {
      ...
      final currentSharedLimit = ref.read(
        sharedCreditLimitForCardProvider(card.id),
      );
  ...
  Future<void> _save() async {
    ...
    final needsSharedLimitAmount =
        (_isEditing && _limitSource == _LimitSource.newSharedLimit) ||
        (!_isEditing && _addNewPairCard);
    final needsOwnCreditLimit =
        !needsSharedLimitAmount &&
        !(_isEditing && _limitSource == _LimitSource.existingSharedLimit);
    ...
    if (needsSharedLimitAmount &&
        double.tryParse(sharedLimitAmountText) == null) {
    ...
    if (addingLinkedCard &&
      ...
      _showSaveError(
        'Enter valid statement and payment due days for the linked card.',
      );
      return;
    }
    if (addingLinkedCard &&
        Validators.lastFourDigits(_pairLastFourDigitsController.text) != null) {
    ...
    try {
      ...
      final minimumDuePercent = double.tryParse(
        _minimumDuePercentController.text.trim(),
      );
      ...
      if (_isEditing) {
        switch (_limitSource) {
          ...
          case _LimitSource.newSharedLimit:
            final sharedLimit = await ref
                .read(sharedCreditLimitRepositoryProvider)
                .createSharedLimit(
      ...
      final cardHolderName = _cardHolderNameController.text.trim().isEmpty
          ? null
      ...
      if (_isEditing) {
        ...
        await repository.editCard(
          ...
          clearMinimumDuePercent: _minimumDuePercentController.text
              .trim()
              .isEmpty,
          autoPay: _autoPay,
          status: _status,
          ...
          autoDebitAccount:
              _autoPay && _autoDebitAccountController.text.trim().isNotEmpty
  ...
  /// The first [AppColors.cardTintPalette] swatch that isn't the primary card's own
  ...
  int _nextDistinctPairColor() {
    return AppColors.cardTintPalette
        .map((c) => c.toARGB32())
        .firstWhere((c) => c != _colorValue, orElse: () => _colorValue);
  }
    ...
    return Material(
      ...
      child: InkWell(
        onTap: () => setState(() {
          ...
          if (_addNewPairCard &&
              _sharedLimitAmountController.text.trim().isEmpty) {
            ...
            children: [
              ...
              Switch(
                ...
                onChanged: (value) => setState(() {
                  ...
                  if (value &&
                      _sharedLimitAmountController.text.trim().isEmpty) {
                    _sharedLimitAmountController.text = _creditLimitController
                        .text
                        .trim();
                  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: AppSizes.md),
          _PremiumField(
            controller: _pairNicknameController,
          ...
          _PremiumDropdown<CardNetwork?>(
            ...
            items: [
              const DropdownMenuItem<CardNetwork?>(
                value: null,
                child: Text('Not set'),
              ),
          ...
          _PremiumField(
            ...
            maxLength: 4,
            validator: (v) =>
                _addNewPairCard ? Validators.lastFourDigits(v) : null,
          ),
          const SizedBox(height: AppSizes.sm),
          Text(
            'Card color',
          ...
          SingleChildScrollView(
            ...
            child: Row(
              children: [
                for (final color in AppColors.cardTintPalette)
                  Padding(
                    padding: const EdgeInsets.only(right: AppSizes.sm),
                    child: GestureDetector(
                      onTap: () =>
                          setState(() => _pairColorValue = color.toARGB32()),
                      child: AnimatedContainer(
  ...
  Widget _buildBasicsStep(BuildContext context) {
    ...
    final existingCards =
        ref.watch(creditCardsStreamProvider).value ?? const [];
    final linkedAccountIds = existingCards
    ...
    final unusedCardAccounts = accounts
        .where(
          (a) => a.type == AccountType.card && !linkedAccountIds.contains(a.id),
        )
        .toList();
        ...
        children: [
          ...
          Container(
            ...
            child: CreditCardVisual(
              ...
              cardHolderName: _cardHolderNameController.text.trim().isEmpty
                  ? null
                  : _cardHolderNameController.text.trim(),
            ),
          ),
            ...
            child: Column(
              ...
              children: [
                ...
                _PremiumField(
                  ...
                  label: 'Card name (optional)',
                  helperText:
                      'A nickname like "Travel card" — otherwise named after the bank',
                  textInputAction: TextInputAction.next,
                  onFieldSubmitted: (_) =>
                      _cardHolderNameFocusNode.requestFocus(),
                ),
                const SizedBox(height: AppSizes.md),
                _PremiumField(
                  ...
                  children: [
                    for (final network in CardNetwork.values)
                      _NetworkChip(
                        ...
                        onTap: () => setState(() {
                          _cardNetwork = _cardNetwork == network
                              ? null
                              : network;
  ...
  Widget _buildLimitStep(BuildContext context) {
    ...
    final selectableSharedLimits = allSharedLimits
        .where((g) => g.id != (_isEditing ? _selectedSharedLimitId : null))
        .toList();
              ...
              children: [
                if (_isEditing) ...[
                  ...
                  SegmentedButton<_LimitSource>(
                    segments: const [
                      ButtonSegment(
                        value: _LimitSource.standalone,
                        label: Text('Own limit'),
                      ),
                      ButtonSegment(
                        value: _LimitSource.newSharedLimit,
                        label: Text('New shared'),
                      ),
                      ButtonSegment(
                        value: _LimitSource.existingSharedLimit,
                        label: Text('Shared'),
                      ),
                    ],
                  ...
                  switch (_limitSource) {
                    ...
                    _LimitSource.existingSharedLimit =>
                      selectableSharedLimits.isEmpty
                          ? Text(
                          ...
                          : _PremiumDropdown<String>(
                              ...
                              items: [
                                for (final sharedLimit
                                    in selectableSharedLimits)
                                  DropdownMenuItem(
                              ...
                              onChanged: (value) => setState(
                                () => _selectedSharedLimitId = value,
                              ),
                              validator: (value) => value == null
                                  ? 'Choose a shared credit limit'
                                  : null,
                            ),
                  },
                ] else
            ...
            child: Column(
              ...
              children: [
                _PremiumField(
                  ...
                  label: 'Bill generated on',
                  helperText:
                      'The day each month your card bill is generated (e.g. 17)',
                  suffixText: 'day of month',
                  keyboardType: TextInputType.number,
            ...
            child: Column(
              ...
              children: [
                ...
                _PremiumField(
                  ...
                  helperText:
                      'The smallest part of your bill you must pay to avoid a late fee. Leave blank if unsure.',
                  suffixText: '%',
                ...
                Material(
                  ...
                  child: SwitchListTile(
                    ...
                    subtitle: const Text(
                      'For your reference only — this app does not make payments automatically.',
                    ),
                    value: _autoPay,
