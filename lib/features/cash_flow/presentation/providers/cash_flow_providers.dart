...
final moneyInLinesForRangeFamilyProvider = Provider.family<List<MoneyFlowLine>, _PeriodRange>((ref, key) {
  ...
  // counterparty paying an installment back to me is Money In, the mirror
  // image of a [LoanDirection.taken] loan's own installment payment being
  // Money Out below in [moneyOutLinesForRangeFamilyProvider]. Bucketed by
  // each [InstallmentPayment]'s own real [InstallmentPayment.date] (the day
  // the repayment actually happened), not the installment's due date — see
  // [moneyOutLinesForRangeFamilyProvider]'s doc comment for why EMI/Bill
  // still use due-date bucketing; a Loan's counterparty-driven repayment has
  // no "cycle" concept to attribute it to, so the real payment date is the
  // only correct event date here.
  for (final loan in ref.watch(activeLoansProvider)) {
    ...
    for (final installment in installments) {
      final payments = ref.watch(
        installmentPaymentsStreamProvider((scheduleId: loan.scheduleId, installmentId: installment.id)),
      ).value ?? const [];
...
final moneyOutLinesForRangeFamilyProvider = Provider.family<List<MoneyFlowLine>, _PeriodRange>((ref, key) {
  ...
  // repayment of money I borrowed is real money leaving me, the same as an
  // EMI installment above. A [LoanDirection.given] loan's installment
  // payment is the counterparty repaying ME, so it's counted in Money In
  // instead (see [moneyInLinesForRangeFamilyProvider]) — never both, since
  // each loan's installments are only ever read from the one direction-gated
  // loop that matches its own [Loan.direction].
  //
  // Bucketed by each [InstallmentPayment]'s own real [InstallmentPayment.date]
  // — unlike EMI/Bill above, which bucket by the installment's due date — so
  // a payment always lands in the range it was actually paid in, matching
  // the "actual financial event/payment date" contract Loan payments must
  // honor (an EMI/Bill's due-date bucketing is a deliberate different
  // choice for scheduled-obligation cycles; a Loan repayment has no such
  // cycle to attribute it to instead of its own payment date).
  for (final loan in ref.watch(activeLoansProvider)) {
