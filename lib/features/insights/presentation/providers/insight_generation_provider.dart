...
/// those providers.
final insightInputsProvider = Provider.family<InsightInputs, InsightPeriod>((
  ref,
  args,
) {
  ...
  double totalFor(DateRange range, TransactionType type) => transactions
      .where(
        (t) => type == t.type && range.contains(args.period.reportDateFor(t)),
      )
  ...
  final categoryEntries = ref.watch(
    categorySpendingBreakdownProvider((range: args.range, period: args.period)),
  );
  final previousCategoryEntries = ref.watch(
    categorySpendingBreakdownProvider((
  ...
  final previousTopCategoryAmount = topCategory == null
      ? null
      : previousCategoryEntries
            .where((e) => e.category.id == topCategory.category.id)
            .firstOrNull
            ?.amount;
...
final generalInsightsProvider = Provider.family<List<Insight>, InsightPeriod>((
  ref,
  args,
) {
...
final healthIndicatorsProvider = Provider.family<List<Insight>, InsightPeriod>((
  ref,
  args,
) {
  final inputs = ref.watch(insightInputsProvider(args));
