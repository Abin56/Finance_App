...
final paymentRepositoryProvider = Provider.autoDispose
    .family<PaymentRepository, String>((ref, billId) {
      final firestore = ref.watch(firestoreProvider);
      final uid = ref.watch(currentUserIdProvider);
      final collection = firestore
          .collection(FirestoreCollections.users)
...
final paymentsStreamProvider = StreamProvider.autoDispose
    .family<List<PaymentRecord>, String>((ref, billId) {
      return ref.watch(paymentRepositoryProvider(billId)).watchAll();
    });

final paymentsTrashStreamProvider = StreamProvider.autoDispose
    .family<List<PaymentRecord>, String>((ref, billId) {
...
final currentOccurrenceByBillIdProvider = Provider<Map<String, BillOccurrence>>(
  (ref) {
...
final overdueBillsProvider = Provider<List<Bill>>((ref) {
  ...
  final overdue =
      bills
          .where((b) => occurrences[b.id]?.status == BillStatus.overdue)
          .toList()
        ..sort(
          (a, b) =>
              occurrences[a.id]!.dueDate.compareTo(occurrences[b.id]!.dueDate),
        );
  return overdue;
});
...
final dueTodayBillsProvider = Provider<List<Bill>>((ref) {
  ...
  final occurrences = ref.watch(currentOccurrenceByBillIdProvider);
  return bills
      .where((b) => occurrences[b.id]?.status == BillStatus.dueToday)
      .toList();
});
...
final upcomingBillsProvider = Provider<List<Bill>>((ref) {
  ...
  final upcoming =
      bills
          .where((b) => occurrences[b.id]?.status == BillStatus.upcoming)
          .toList()
        ..sort(
          (a, b) =>
              occurrences[a.id]!.dueDate.compareTo(occurrences[b.id]!.dueDate),
...
final paidBillsProvider = Provider<List<Bill>>((ref) {
  ...
  final occurrences = ref.watch(currentOccurrenceByBillIdProvider);
  return bills
      .where((b) => occurrences[b.id]?.status == BillStatus.paid)
      .toList();
});
...
final totalOverdueAmountProvider = Provider<double>((ref) {
  ...
  return ref
      ...
      .fold(
        0.0,
        (total, bill) => total + occurrences[bill.id]!.remainingAmount,
      );
});
