...
class _ReportsScreenState extends ConsumerState<ReportsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final range = _period.rangeFor(
      now,
      fiscalYearStartMonth: fiscalYearStartMonth,
    );
    // Every Reports figure/chart on this screen must filter from this same
    // list (via `_period.reportDateFor`, not a re-derived date) so nothing
    // can silently disagree about which transactions are "in period".
    final periodTransactions = transactions
        .where((t) => range.contains(_period.reportDateFor(t)))
        .toList();

    final previousRange = _previousRangeFor(_period, now, fiscalYearStartMonth);
    final previousTransactions = transactions
        .where((t) => previousRange.contains(_period.reportDateFor(t)))
        .toList();

    double totalFor(List<Transaction> list, TransactionType type) => list
        .where((t) => t.type == type)
    ...
    final myExpenseBreakdown = ref.watch(
      myExpenseBreakdownForTransactionsProvider(periodTransactions),
    );
    ...
    final categoryEntries = ref.watch(
      categorySpendingBreakdownProvider((range: range, period: _period)),
    );
    final insightArgs = (
      range: range,
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: transactions.isEmpty && emis.isEmpty
            ? const EmptyState(
            ...
            : ListView(
                ...
                children: [
                  ...
                  ReportsOverviewCard(
                    ...
                    expensesChangePercent: _percentChange(
                      expenses,
                      prevExpenses,
                    ),
                    netSavingsChangePercent: _percentChange(
                      netSavings,
                      prevNetSavings,
                    ),
                  ),
                  ...
                  if (categoryEntries.isEmpty)
                    ...
                    ReportsCategoryList(
                      ...
                      onTapCategory: (category) => context.push(
                        '${AppRoutes.reports}/category/${category.id}?period=${_period.name}',
                      ),
                    ),
                  ],
  ...
  DateRange _previousRangeFor(
    ...
    switch (period) {
      ...
      case ReportsPeriod.financialYear:
        ...
        return ReportsPeriod.financialYear.rangeFor(
          oneYearAgo,
          fiscalYearStartMonth: fiscalYearStartMonth,
        );
