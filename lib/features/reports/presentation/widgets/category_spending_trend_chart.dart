...
class CategorySpendingTrendChart extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalsByDay = {
      for (final d in days)
        d: transactions
            .where((t) => period.reportDateFor(t).dateOnly == d)
            .fold(0.0, (sum, t) => sum + t.amount),
    };

    final maxValue = totalsByDay.values.fold(
      0.0,
      (max, v) => v > max ? v : max,
    );
    ...
    final labelEvery = (days.length / 6).ceil().clamp(
      1,
                  ...
                  LineChartBarData(
                    spots: [
                      for (var i = 0; i < days.length; i++)
                        FlSpot(i.toDouble(), totalsByDay[days[i]] ?? 0),
                    ],
                    isCurved: true,
                    color: color,
                    barWidth: 3,
