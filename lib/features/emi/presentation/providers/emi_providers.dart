...
final emiPaymentBreakdownRepositoryProvider = Provider.autoDispose
    .family<EmiPaymentBreakdownRepository, String>((ref, emiId) {
      final firestore = ref.watch(firestoreProvider);
      final uid = ref.watch(currentUserIdProvider);
      final collection = firestore
          .collection(FirestoreCollections.users)
...
final emiPaymentBreakdownsStreamProvider = StreamProvider.autoDispose
    .family<List<EmiPaymentBreakdown>, String>((ref, emiId) {
...
final emiStatusProvider = Provider.autoDispose.family<EmiStatus, Emi>((
  ref,
  emi,
) {
  final installments =
...
final emiRemainingAmountProvider = Provider.autoDispose.family<double, Emi>((
  ref,
  emi,
) {
...
final emiTotalPaidProvider = Provider.autoDispose.family<double, Emi>((
  ref,
  emi,
) {
  final installments =
...
final emiInstallmentsPaidProvider = Provider.autoDispose.family<int, Emi>((
  ref,
  emi,
) {
  final installments =
...
final emiRemainingTenureProvider = Provider.autoDispose.family<int, Emi>((
  ref,
  emi,
) {
  final installments =
...
/// the same figure.
final emiPrincipalOutstandingProvider = Provider.autoDispose
    .family<double, Emi>((ref, emi) {
      final installments =
...
final emiInterestOutstandingProvider = Provider.autoDispose.family<double, Emi>(
  (ref, emi) {
    final installments =
        ref.watch(installmentsStreamProvider(emi.scheduleId)).value ?? const [];
    return installments.fold(0.0, (sum, i) {
...
final emiTotalInterestPayableProvider = Provider.autoDispose
    .family<double, Emi>((ref, emi) {
      final installments =
          ref.watch(installmentsStreamProvider(emi.scheduleId)).value ??
          const [];
...
/// progress ring/bar.
final emiLoanProgressProvider = Provider.autoDispose.family<double, Emi>((
  ref,
  emi,
) {
  final installments =
...
final overdueEmisProvider = Provider<List<Emi>>((ref) {
  final emis = ref.watch(emisStreamProvider).value ?? const [];
  return emis
      .where((e) => ref.watch(emiStatusProvider(e)) == EmiStatus.overdue)
      .toList();
});
...
final totalRemainingEmiBalanceProvider = Provider<double>((ref) {
  ...
  return emis.fold(
    0.0,
    (sum, emi) => sum + ref.watch(emiRemainingAmountProvider(emi)),
  );
});
...
final emiPaymentHistoryProvider = Provider.autoDispose
    .family<List<EmiPaymentHistoryEntry>, Emi>((ref, emi) {
      final installments =
          ref.watch(installmentsStreamProvider(emi.scheduleId)).value ??
          const [];
...
final nextEmiDueProvider = Provider<({Emi emi, Installment installment})?>((
  ref,
) {
  final emis = ref.watch(activeEmisProvider);
  ...
  for (final emi in emis) {
    final installments =
        ref.watch(installmentsStreamProvider(emi.scheduleId)).value ?? const [];
    final unpaid = installments.where(
      (i) => i.status != InstallmentStatus.paid && !i.isSkipped,
    );
    for (final installment in unpaid) {
      if (next == null ||
          installment.dueDate.isBefore(next.installment.dueDate)) {
...
final emiCycleViewProvider = Provider.autoDispose
    .family<CycleEngineResult<InstallmentCycleItem>, Emi>((ref, emi) {
      final installments =
          ref.watch(installmentsStreamProvider(emi.scheduleId)).value ??
          const [];
...
typedef EmiCycleView = ({
  List<Installment> previousCyclePending,
  List<Installment> current,
});

final emiCycleViewRecordProvider = Provider.autoDispose
    .family<EmiCycleView, Emi>((ref, emi) {
