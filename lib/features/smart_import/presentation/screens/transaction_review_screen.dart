...
import '../widgets/detected_transaction_edit_sheet.dart';
import '../widgets/detected_transaction_tile.dart';

/// The staged review the spec calls out as the most important part of Smart
/// Import — nothing here has touched Firestore yet. The user picks an
/// account, reviews/edits each detected row, resolves duplicate warnings,
...
class TransactionReviewScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ref.listen(smartImportControllerProvider, (previous, next) {
      if (next.errorMessage != null &&
          next.errorSequence != previous?.errorSequence) {
    ...
    return PopScope(
      canPop: state.stage != SmartImportStage.importing,
      child: Scaffold(
        appBar: AppBar(
          title: Text(_titleFor(state)),
          automaticallyImplyLeading: state.stage != SmartImportStage.importing,
        ),
        body: switch (state.stage) {
          SmartImportStage.importing => _ImportingView(state: state),
          SmartImportStage.done => _ImportSummaryScreenBody(state: state),
  ...
  String _titleFor(SmartImportState state) => switch (state.stage) {
    SmartImportStage.done => 'Import complete',
    SmartImportStage.importing => 'Importing…',
    _ => 'Smart Import',
  };
}
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (state.detected.isEmpty) {
      return const EmptyState(
        ...
        subtitle: 'Make sure the screenshot contains transaction details.',
      );
    }

    // Mirrors `SmartImportState.readyCount` exactly — what will actually be
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return DetectedTransactionTile(
                ...
                onTap: () =>
                    DetectedTransactionEditSheet.show(context, transaction),
