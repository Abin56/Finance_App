...
class DashboardLayoutController extends Notifier<DashboardState> {
  ...
  Future<void> removeWidget(String dashboardWidgetId) async {
    final layout = state.activeLayout;
    final widgets = layout.widgets
        .where((w) => w.id != dashboardWidgetId)
        .toList();
    await _updateActiveLayout(layout.copyWith(widgets: widgets));
  }
  ...
  Future<void> addWidget(
    ...
    WidgetConfiguration? seed,
  }) async {
    final configId = _uuid.v4();
    final widgetId = _uuid.v4();
    final source =
        seed ??
        WidgetConfiguration(id: configId, type: type, title: type.defaultTitle);
    ...
    final widgets = [
      ...layout.widgets,
      DashboardWidget(id: widgetId, type: type, configId: configId),
    ];
    ...
    await _persist(
      state.copyWith(
        ...
        layouts: [
          for (final l in state.layouts)
            l.id == layout.id ? layout.copyWith(widgets: widgets) : l,
        ],
      ),
    );
  }
  ...
  Future<void> _updateConfig(WidgetConfiguration updated) async {
    await _persist(
      state.copyWith(configs: {...state.configs, updated.id: updated}),
    );
  }
  ...
  Future<void> _updateActiveLayout(DashboardLayout updated) async {
    await _persist(
      state.copyWith(
        layouts: [
          for (final l in state.layouts) l.id == updated.id ? updated : l,
        ],
      ),
    );
  }
...
final dashboardLayoutControllerProvider =
    NotifierProvider<DashboardLayoutController, DashboardState>(
...
final visibleDashboardWidgetsProvider =
    Provider<List<(DashboardWidget, WidgetConfiguration)>>((ref) {
